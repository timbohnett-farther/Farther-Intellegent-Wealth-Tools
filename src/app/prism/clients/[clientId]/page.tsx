...
const tierColors: Record<string, string> = {
  emerging: 'bg-gray-100 text-gray-600', mass_affluent: 'bg-blue-100 text-blue-700',
  hnw: 'bg-purple-100 text-purple-700', uhnw: 'bg-amber-100 text-amber-700',
};
...
export default function ClientDetailPage() {
  ...
  return (
    <div className="max-w-content mx-auto flex">
      ...
      <div className="w-[260px] flex-shrink-0 p-6 border-r border-gray-200 bg-white min-h-[calc(100vh-3.5rem)]">
        {/* Avatar */}
        <div className="flex flex-col items-center mb-5">
          ...
          <h2 className="text-lg font-bold text-gray-900">{client.firstName} {client.lastName}</h2>
          {client.coClient && (
            <p className="text-sm text-gray-500">& {client.coClient}</p>
          )}
        ...
        <div className="space-y-3 text-sm">
          <div className="flex items-start gap-2">
            ...
            <div>
              <p className="text-gray-500 text-xs">Age / DOB</p>
              <p className="text-gray-900">{client.age} · {client.dob}</p>
            </div>
          </div>
          <div className="flex items-start gap-2">
            <Shield size={14} className="text-gray-400 mt-0.5" />
            <div>
              <p className="text-gray-500 text-xs">Advisor</p>
              <p className="text-gray-900">{client.advisor}</p>
            </div>
          </div>
          <div className="flex items-start gap-2">
          ...
          <div className="flex items-start gap-2">
            ...
            <div>
              ...
              <p className="text-gray-900 text-xs break-all">{client.email}</p>
            </div>
          </div>
          <div className="flex items-start gap-2">
            ...
            <div>
              ...
              <p className="text-gray-900 text-xs">{client.address}</p>
            </div>
          </div>
          <div className="flex items-start gap-2">
            ...
            <div>
              ...
              <p className="text-gray-900">{client.clientSince}</p>
            </div>
          </div>
          <div className="flex items-start gap-2">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1">
                {client.tags.map(tag => (
                  <span key={tag} className="inline-block px-2 py-0.5 rounded bg-gray-100 text-[10px] font-medium text-gray-600">{tag}</span>
                ))}
              </div>
            </div>
          </div>
        ...
        <div className="p-6">
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
                  ...
                  <p className="text-xl font-bold text-green-600" style={{ fontFeatureSettings: '"tnum"' }}>{formatCurrency(client.totalAssets)}</p>
                </div>
                <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
                  <p className="text-xs text-gray-500 mb-1">Total Liabilities</p>
                  <p className="text-xl font-bold text-red-500" style={{ fontFeatureSettings: '"tnum"' }}>{formatCurrency(client.totalLiabilities)}</p>
                </div>
                <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
                  <p className="text-xs text-gray-500 mb-1">Net Worth</p>
                  <p className="text-xl font-bold text-gray-900" style={{ fontFeatureSettings: '"tnum"' }}>{formatCurrency(client.netWorth)}</p>
                </div>
              </div>
              ...
              <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
                <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
                  <h3 className="font-semibold text-gray-900">Financial Plans</h3>
                ...
                <div className="divide-y divide-gray-50">
                  {client.plans.map((plan) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium text-gray-900 group-hover:text-brand-600">{plan.name}</p>
                      ...
                      {plan.successRate !== null && (
                        <span className="text-sm font-medium text-gray-700" style={{ fontFeatureSettings: '"tnum"' }}>
                          {plan.successRate}%
                        </span>
                      )}
          ...
          {activeTab === 'Personal Details' && (
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
              <p className="text-sm text-gray-500">Personal details form will be displayed here. This form allows editing all client demographic, contact, employment, and planning preference fields.</p>
            </div>
          )}

          {activeTab === 'Accounts & Net Worth' && (
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
              <p className="text-sm text-gray-500">Account list with balances, tax bucket breakdown, and net worth visualization will be displayed here.</p>
            </div>
          )}

          {activeTab === 'Documents' && (
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
              <p className="text-sm text-gray-500">Document vault for uploaded files, estate documents, and policy documents will be displayed here.</p>
            </div>
          )}
              ...
              <div className="space-y-3">
                {client.recentActivity.map((item, i) => (
                  <div key={i} className="flex items-center gap-3 py-2 border-b border-gray-50 last:border-0">
                    ...
                    <p className="text-sm text-gray-700 flex-1">{item.event}</p>
          ...
          {activeTab === 'Risk Profile' && (
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
              <p className="text-sm text-gray-500">Risk profile assessment and results. Links to Farther Focus risk profiling tool for full assessment.</p>
              <Link
                href="/risk-profile"
      ...
      <div className="w-[300px] flex-shrink-0 p-6 border-l border-gray-200 bg-white min-h-[calc(100vh-3.5rem)] hidden xl:block">
        <h3 className="font-semibold text-gray-900 mb-4">Activity & Alerts</h3>

        <div className="space-y-4">
          <div className="p-3 rounded-lg bg-amber-50 border border-amber-200">
            <div className="flex items-center gap-2 mb-1">
              <AlertCircle size={14} className="text-amber-600" />
              <p className="text-xs font-semibold text-amber-700">Review Overdue</p>
            </div>
            <p className="text-xs text-amber-600">Annual review was due on Feb 15, 2026.</p>
          </div>

          <div>
            <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Recent Updates</h4>
