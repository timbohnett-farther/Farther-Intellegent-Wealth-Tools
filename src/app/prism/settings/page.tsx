...
export default function SettingsPage() {
  return (
    <div className="max-w-content mx-auto px-6 py-6">
      ...
      <p className="text-sm text-charcoal-500 mb-8">Configure your Farther Prism platform settings.</p>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {SETTINGS_SECTIONS.map((section) => (
          <div key={section.title} className="bg-white rounded-xl border border-limestone-200 shadow-sm p-5 opacity-60">
            <div className="flex items-center gap-3 mb-2">
              <div className="w-10 h-10 rounded-lg bg-limestone-100 flex items-center justify-center text-charcoal-300">
                {section.icon}
              </div>
              <div>
                <h3 className="font-semibold text-charcoal-900">{section.title}</h3>
                <span className="text-[10px] text-charcoal-300 font-medium">COMING SOON</span>
              </div>
            </div>
            <p className="text-sm text-charcoal-500">{section.description}</p>
          </div>
        ))}
      </div>
    </div>
