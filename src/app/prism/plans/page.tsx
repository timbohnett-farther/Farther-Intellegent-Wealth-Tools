...
const statusColors: Record<string, string> = {
  ...
  needs_review: 'bg-amber-100 text-amber-700', archived: 'bg-gray-100 text-gray-500',
};
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {filtered.map((plan) => (
          <Link
            ...
            className="bg-white rounded-xl border border-gray-200 shadow-sm p-5 hover:shadow-md hover:border-brand-200 transition-all group"
          >
            ...
            <h3 className="font-semibold text-gray-900 mb-0.5 group-hover:text-brand-600 transition-colors">{plan.planName}</h3>
            <p className="text-sm text-gray-500 mb-3">{plan.clientName}</p>

            <div className="flex items-center justify-between text-xs">
              <span className="text-gray-400">Updated {plan.lastUpdated}</span>
              {plan.successRate !== null && (
                <span className={`font-semibold ${plan.successRate >= 85 ? 'text-green-600' : plan.successRate >= 70 ? 'text-amber-600' : 'text-red-600'}`}
            ...
            <div className="mt-3">
              <div className="flex items-center justify-between text-[10px] mb-1">
                ...
                <span className="text-gray-500 font-medium" style={{ fontFeatureSettings: '"tnum"' }}>{plan.completionScore}%</span>
              </div>
              <div className="w-full h-1.5 bg-gray-100 rounded-full overflow-hidden">
                <div
                  className={`h-full rounded-full ${plan.completionScore >= 90 ? 'bg-green-500' : plan.completionScore >= 60 ? 'bg-brand-500' : plan.completionScore >= 40 ? 'bg-amber-500' : 'bg-gray-400'}`}
