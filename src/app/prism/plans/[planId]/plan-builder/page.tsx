  ...
  return (
    <div className="flex h-[calc(100vh-120px)] bg-transparent">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {/* Chat Header */}
        <div className="bg-surface-soft border-b px-6 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-lg bg-accent-primary/15 flex items-center justify-center">
              <MessageSquare size={18} className="text-accent-primarySoft" />
            </div>
            <div>
              <h2 className="text-sm font-semibold text-text">AI Plan Builder</h2>
              <p className="text-xs text-text-muted">Conversational data intake</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <span className="text-xs text-text-muted">Plan: {planId?.slice(0, 8)}...</span>
          </div>
        </div>
          ...
          {!sessionStarted ? (
            <div className="flex flex-col items-center justify-center h-full text-center">
              <div className="w-16 h-16 rounded-2xl bg-accent-primary/15 flex items-center justify-center mb-4">
                <Bot size={32} className="text-accent-primarySoft" />
              </div>
              <h3 className="text-lg font-semibold text-text mb-2">Plan Builder AI Assistant</h3>
              ...
              <button
                ...
                className="px-6 py-2.5 bg-accent-primary text-text rounded-lg text-sm font-medium hover:bg-accent-primary transition-colors flex items-center gap-2"
              >
                <Play size={16} />
              ...
              <div className="mt-6 grid grid-cols-2 gap-3 max-w-md">
                {[
                  ...
                  <button
                    key={i}
                    className="flex items-center gap-2 px-3 py-2 bg-surface-soft border border-border-subtle rounded-lg text-xs text-text-muted hover:bg-surface-subtle transition-colors"
                  >
                    {item.icon}
                    {item.text}
                  </button>
            ...
            <>
                ...
                >
                  <div
                    className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${
                      msg.role === 'ai' ? 'bg-accent-primary/15' : 'bg-surface-subtle'
                    }`}
                  >
                    {msg.role === 'ai' ? (
                      <Bot size={16} className="text-accent-primarySoft" />
                    ) : (
                  ...
                  <div
                    className={`max-w-[70%] rounded-xl px-4 py-3 text-sm ${
                      msg.role === 'ai'
                        ? 'bg-surface-soft border border-border-subtle text-text'
              ...
              {pendingConfirmation && (
                <div className="flex gap-2 ml-11">
                  <button
                    onClick={confirmData}
                    className="px-4 py-2 bg-success-500 text-text rounded-lg text-sm font-medium hover:bg-success-700 transition-colors flex items-center gap-1.5"
                  >
                  ...
                  <button
                    ...
                    className="px-4 py-2 bg-surface-soft border border-border-subtle text-text-muted rounded-lg text-sm hover:bg-surface-subtle transition-colors flex items-center gap-1.5"
                  >
                    <X size={14} />
                    Edit
                  </button>
              ...
              {isProcessing && (
                <div className="flex gap-3">
                  ...
                  <div className="bg-surface-soft border border-border-subtle rounded-xl px-4 py-3">
        ...
        {/* Input Area */}
        {sessionStarted && (
          <div className="bg-surface-soft border-t px-6 py-4">
            <div className="flex gap-3">
              <button className="p-2.5 text-text-faint hover:text-text-muted border border-border-subtle rounded-lg hover:bg-surface-subtle transition-colors">
                <Upload size={18} />
              </button>
              <div className="flex-1 relative">
                <input
                  ...
                  className="w-full px-4 py-2.5 border border-border-subtle rounded-lg text-sm focus:outline-hidden focus:ring-2 focus:ring-accent-primary focus:border-transparent"
              ...
              <button
                ...
                className="p-2.5 bg-accent-primary text-text rounded-lg hover:bg-accent-primary transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <Send size={18} />
              </button>
            </div>
      ...
      <div className="w-80 bg-surface-soft backdrop-blur-xl border-l flex flex-col">
        <div className="px-5 py-4 border-b">
          <h3 className="text-sm font-semibold text-text">Plan Completion</h3>
          <div className="mt-3">
            <div className="flex items-center justify-between text-xs text-text-muted mb-1">
              <span>Overall Progress</span>
              <span className="font-medium text-text">{completionScore}%</span>
            </div>
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 px-5 py-3 text-left transition-colors ${
                  isActive ? 'bg-accent-primary/10 border-r-2 border-accent-primary' : 'hover:bg-surface-subtle'
                }`}
              >
                <div
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="w-full h-1 bg-surface-subtle rounded-full mt-1.5 overflow-hidden">
                    <div
                      className={`h-full rounded-full transition-all duration-300 ${
                        isComplete ? 'bg-success-500' : 'bg-accent-primary'
                      }`}
                      style={{ width: `${pct}%` }}
                    />
                  </div>
        ...
        <div className="border-t px-5 py-4">
          <h4 className="text-xs font-semibold text-text-muted mb-2">Quick Upload</h4>
          <div className="border-2 border-dashed border-border-subtle rounded-lg p-4 text-center hover:border-accent-primarySoft transition-colors cursor-pointer">
            <Upload size={20} className="mx-auto text-text-faint mb-1" />
