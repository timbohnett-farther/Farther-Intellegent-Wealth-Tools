  ...
  function PieTooltip({ active, payload }: { active?: boolean; payload?: Array<{ name: string; value: number }> }) {
    ...
    return (
      <div className="rounded-md border border-limestone-200 bg-white px-3 py-2 text-xs shadow-lg">
        ...
        <p className="text-charcoal-700">{fmt$(payload[0].value)}</p>
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      <div className="max-w-content mx-auto px-6 py-6">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <h1 className="text-xl font-bold text-charcoal-900">Equity Compensation</h1>
            </div>
        ...
        {/* Tab Navigation */}
        <div className="border-b border-limestone-200 mb-6">
              ...
              <button
                ...
                className={`whitespace-nowrap border-b-2 px-3 py-2.5 text-xs font-medium transition-colors ${
                  activeTab === tab
                    ...
                    : 'border-transparent text-charcoal-500 hover:border-limestone-300 hover:text-charcoal-700'
                }`}
              >
                {tab}
              </button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-white rounded-xl border border-limestone-200 shadow-sm p-5">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-xs font-medium text-charcoal-500 uppercase tracking-wide">Total Equity Value</span>
                </div>
                <p className="text-2xl font-bold text-charcoal-900">{fmt$(totalEquityValue)}</p>
              ...
              <div className="bg-white rounded-xl border border-limestone-200 shadow-sm p-5">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-xs font-medium text-charcoal-500 uppercase tracking-wide">Unrealized Gain</span>
                </div>
                <p className="text-2xl font-bold text-success-500">{fmt$(totalUnrealizedGain)}</p>
                <p className="text-xs text-charcoal-500 mt-1">Cost basis: {fmt$(totalCostBasis)}</p>
              </div>
              <div className="bg-white rounded-xl border border-limestone-200 shadow-sm p-5">
                <div className="flex items-center gap-2 mb-1">
                  <PieChartIcon size={16} className="text-indigo-500" />
                  <span className="text-xs font-medium text-charcoal-500 uppercase tracking-wide">% of Portfolio</span>
                </div>
                <p className="text-2xl font-bold text-charcoal-900">{fmtPct(equityPctOfPortfolio)}</p>
                <p className="text-xs text-charcoal-500 mt-1">Portfolio: {fmt$(portfolioValue)}</p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 bg-white rounded-xl border border-limestone-200 shadow-sm">
                <div className="px-5 py-4 border-b border-limestone-100">
                  <h3 className="text-sm font-semibold text-charcoal-900">Holdings Detail</h3>
                </div>
                <div className="overflow-x-auto">
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="border-b border-limestone-100 bg-limestone-50">
                        <th className="px-4 py-2.5 text-left font-medium text-charcoal-500">Type</th>
                        <th className="px-4 py-2.5 text-left font-medium text-charcoal-500">Company</th>
                        <th className="px-4 py-2.5 text-left font-medium text-charcoal-500">Ticker</th>
                        <th className="px-4 py-2.5 text-right font-medium text-charcoal-500">Shares</th>
                        <th className="px-4 py-2.5 text-right font-medium text-charcoal-500">Price</th>
                        <th className="px-4 py-2.5 text-right font-medium text-charcoal-500">Value</th>
                        <th className="px-4 py-2.5 text-right font-medium text-charcoal-500">Gain</th>
                        <th className="px-4 py-2.5 text-left font-medium text-charcoal-500">Status</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-limestone-50">
                      {rsuGrants.map((g) => (
                        <tr key={`rsu-${g.id}`} className="hover:bg-limestone-50">
                          ...
                          <td className="px-4 py-2.5 font-medium text-charcoal-900">{g.company}</td>
                          <td className="px-4 py-2.5 text-charcoal-700">{g.ticker}</td>
                          <td className="px-4 py-2.5 text-right tabular-nums text-charcoal-900">{g.sharesHeld}</td>
                          <td className="px-4 py-2.5 text-right tabular-nums text-charcoal-900">${g.currentFMV.toFixed(2)}</td>
                          <td className="px-4 py-2.5 text-right tabular-nums font-medium text-charcoal-900">{fmt$(g.sharesHeld * g.currentFMV)}</td>
                      ...
                      {options.map((o) => (
                        <tr key={`opt-${o.id}`} className="hover:bg-limestone-50">
                          <td className="px-4 py-2.5"><span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-medium ${o.type === 'ISO' ? 'bg-brand-50 text-brand-700' : 'bg-brand-50 text-brand-700'}`}>{o.type}</span></td>
                          <td className="px-4 py-2.5 font-medium text-charcoal-900">{o.company}</td>
                          <td className="px-4 py-2.5 text-charcoal-700">{o.ticker}</td>
                          <td className="px-4 py-2.5 text-right tabular-nums text-charcoal-900">{o.shares}</td>
                          <td className="px-4 py-2.5 text-right tabular-nums text-charcoal-900">${o.currentFMV.toFixed(2)}</td>
                          <td className="px-4 py-2.5 text-right tabular-nums font-medium text-charcoal-900">{fmt$(Math.max(0, o.currentFMV - o.exercisePrice) * o.shares)}</td>
                      ...
                      {esppPurchases.map((p) => (
                        <tr key={`espp-${p.id}`} className="hover:bg-limestone-50">
                          <td className="px-4 py-2.5"><span className="inline-flex items-center rounded-full bg-brand-50 px-2 py-0.5 text-[10px] font-medium text-brand-700">ESPP</span></td>
                          <td className="px-4 py-2.5 font-medium text-charcoal-900">{p.company}</td>
                          <td className="px-4 py-2.5 text-charcoal-700">{p.ticker}</td>
                          <td className="px-4 py-2.5 text-right tabular-nums text-charcoal-900">{p.shares}</td>
                          <td className="px-4 py-2.5 text-right tabular-nums text-charcoal-900">${p.currentFMV.toFixed(2)}</td>
                          <td className="px-4 py-2.5 text-right tabular-nums font-medium text-charcoal-900">{fmt$(p.shares * p.currentFMV)}</td>
              ...
              {/* Pie Chart */}
              <div className="bg-white rounded-xl border border-limestone-200 shadow-sm p-5">
                <h3 className="text-sm font-semibold text-charcoal-900 mb-4">Allocation by Type</h3>
                {allocationData.length > 0 ? (
                  ...
                  <div className="flex items-center justify-center h-[220px] text-xs text-charcoal-300">No holdings data</div>
                )}
                <div className="mt-3 space-y-2">
                  {allocationData.map((d, idx) => (
                    <div key={d.name} className="flex items-center justify-between text-xs">
                      ...
                      <span className="font-medium tabular-nums text-charcoal-900">{fmt$(d.value)}</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl border border-limestone-200 shadow-sm">
              <div className="px-5 py-4 border-b border-limestone-100 flex items-center justify-between">
                <h3 className="text-sm font-semibold text-charcoal-900">RSU Grants</h3>
                <button
              ...
              {/* Add RSU Form */}
              {showAddRSU && (
                <div className="px-5 py-4 border-b border-limestone-100 bg-limestone-50">
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    <div>
                      <label className="block text-[10px] font-medium text-charcoal-500 uppercase mb-1">Company</label>
                      <input type="text" value={newRSU.company || ''} onChange={e => setNewRSU({ ...newRSU, company: e.target.value })}
                        className="w-full rounded-md border border-limestone-300 px-2.5 py-1.5 text-xs focus:border-brand-500 focus:ring-1 focus:ring-brand-500" placeholder="Alphabet Inc." />
                    </div>
                    <div>
                      <label className="block text-[10px] font-medium text-charcoal-500 uppercase mb-1">Ticker</label>
                      <input type="text" value={newRSU.ticker || ''} onChange={e => setNewRSU({ ...newRSU, ticker: e.target.value })}
                        className="w-full rounded-md border border-limestone-300 px-2.5 py-1.5 text-xs focus:border-brand-500 focus:ring-1 focus:ring-brand-500" placeholder="GOOGL" />
                    </div>
                    <div>
                      <label className="block text-[10px] font-medium text-charcoal-500 uppercase mb-1">Grant Date</label>
                      <input type="date" value={newRSU.grantDate || ''} onChange={e => setNewRSU({ ...newRSU, grantDate: e.target.value })}
                        className="w-full rounded-md border border-limestone-300 px-2.5 py-1.5 text-xs focus:border-brand-500 focus:ring-1 focus:ring-brand-500" />
                    </div>
                    <div>
                      <label className="block text-[10px] font-medium text-charcoal-500 uppercase mb-1">Shares Granted</label>
                      <input type="number" value={newRSU.sharesGranted || ''} onChange={e => setNewRSU({ ...newRSU, sharesGranted: Number(e.target.value) })}
                        className="w-full rounded-md border border-limestone-300 px-2.5 py-1.5 text-xs focus:border-brand-500 focus:ring-1 focus:ring-brand-500" placeholder="100" />
                    </div>
                    <div>
                      <label className="block text-[10px] font-medium text-charcoal-500 uppercase mb-1">Grant Price</label>
                      <input type="number" step="0.01" value={newRSU.grantPrice || ''} onChange={e => setNewRSU({ ...newRSU, grantPrice: Number(e.target.value) })}
                        className="w-full rounded-md border border-limestone-300 px-2.5 py-1.5 text-xs focus:border-brand-500 focus:ring-1 focus:ring-brand-500" placeholder="150.00" />
                    </div>
                    <div>
                      <label className="block text-[10px] font-medium text-charcoal-500 uppercase mb-1">Vesting Schedule</label>
                      <input type="text" value={newRSU.vestingSchedule || ''} onChange={e => setNewRSU({ ...newRSU, vestingSchedule: e.target.value })}
                        className="w-full rounded-md border border-limestone-300 px-2.5 py-1.5 text-xs focus:border-brand-500 focus:ring-1 focus:ring-brand-500" placeholder="4yr / quarterly" />
                    </div>
                    <div>
                      <label className="block text-[10px] font-medium text-charcoal-500 uppercase mb-1">Current FMV</label>
                      <input type="number" step="0.01" value={newRSU.currentFMV || ''} onChange={e => setNewRSU({ ...newRSU, currentFMV: Number(e.target.value) })}
                        className="w-full rounded-md border border-limestone-300 px-2.5 py-1.5 text-xs focus:border-brand-500 focus:ring-1 focus:ring-brand-500" placeholder="178.25" />
                    </div>
                    <div className="flex items-end">
                      <button onClick={addRSUGrant}
                        className="w-full rounded-md bg-brand-500 px-3 py-1.5 text-xs font-medium text-white hover:bg-brand-600 transition-colors">
                        Save Grant
                      </button>
                    </div>
                  </div>
              ...
              {rsuGrants.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-12 text-center">
                  ...
                  <p className="text-xs text-charcoal-500">No RSU grants added yet. Click &quot;Add RSU Grant&quot; to get started.</p>
                </div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="border-b border-limestone-100 bg-limestone-50">
                        <th className="px-4 py-2.5 text-left font-medium text-charcoal-500">Company</th>
                        <th className="px-4 py-2.5 text-left font-medium text-charcoal-500">Ticker</th>
                        <th className="px-4 py-2.5 text-left font-medium text-charcoal-500">Grant Date</th>
                        <th className="px-4 py-2.5 text-right font-medium text-charcoal-500">Granted</th>
                        <th className="px-4 py-2.5 text-right font-medium text-charcoal-500">Grant Price</th>
                        <th className="px-4 py-2.5 text-left font-medium text-charcoal-500">Schedule</th>
                        <th className="px-4 py-2.5 text-right font-medium text-charcoal-500">Vested</th>
                        <th className="px-4 py-2.5 text-right font-medium text-charcoal-500">Held</th>
                        <th className="px-4 py-2.5 text-right font-medium text-charcoal-500">FMV</th>
                        <th className="px-4 py-2.5 text-right font-medium text-charcoal-500">Value</th>
                    ...
                    <tbody className="divide-y divide-limestone-50">
                      {rsuGrants.map((g) => (
                        <tr key={g.id} className={`hover:bg-limestone-50 cursor-pointer ${selectedRSU === g.id ? 'bg-indigo-50' : ''}`}
                          onClick={() => setSelectedRSU(selectedRSU === g.id ? null : g.id)}>
                          <td className="px-4 py-2.5 font-medium text-charcoal-900">{g.company}</td>
                          <td className="px-4 py-2.5 text-charcoal-700">{g.ticker}</td>
                          <td className="px-4 py-2.5 text-charcoal-700">{g.grantDate}</td>
                          ...
                          <td className="px-4 py-2.5 text-charcoal-700">{g.vestingSchedule}</td>
                          ...
                          <td className="px-4 py-2.5 text-right">
                            <button onClick={(e) => { e.stopPropagation(); removeRSUGrant(g.id); }}
                              className="text-charcoal-300 hover:text-critical-500 transition-colors"><Trash2 size={14} /></button>
                          </td>
                        </tr>
                      ))}
                    </tbody>
            ...
            <div className="bg-white rounded-xl border border-limestone-200 shadow-sm">
              <div className="px-5 py-4 border-b border-limestone-100">
                <h3 className="text-sm font-semibold text-charcoal-900">Per-Vest Tax Calculator</h3>
                <p className="text-[10px] text-charcoal-500 mt-0.5">RSUs are taxed as ordinary income (W-2) on the vesting date</p>
              </div>
              <div className="p-5">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                  <div>
                    <label className="block text-[10px] font-medium text-charcoal-500 uppercase mb-1">Shares Vesting</label>
                    <input type="number" value={rsuVestShares} onChange={e => setRsuVestShares(Number(e.target.value))}
                      className="w-full rounded-md border border-limestone-300 px-2.5 py-1.5 text-xs focus:border-brand-500 focus:ring-1 focus:ring-brand-500" />
                  </div>
                  <div>
                    <label className="block text-[10px] font-medium text-charcoal-500 uppercase mb-1">FMV / Share</label>
                    <input type="number" step="0.01" value={rsuVestFMV} onChange={e => setRsuVestFMV(Number(e.target.value))}
                      className="w-full rounded-md border border-limestone-300 px-2.5 py-1.5 text-xs focus:border-brand-500 focus:ring-1 focus:ring-brand-500" />
                  </div>
                  <div>
                    <label className="block text-[10px] font-medium text-charcoal-500 uppercase mb-1">Federal Rate %</label>
                    <input type="number" value={rsuFedRate} onChange={e => setRsuFedRate(Number(e.target.value))}
                      className="w-full rounded-md border border-limestone-300 px-2.5 py-1.5 text-xs focus:border-brand-500 focus:ring-1 focus:ring-brand-500" />
                  </div>
                  <div>
                    <label className="block text-[10px] font-medium text-charcoal-500 uppercase mb-1">State Rate %</label>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-3">
                    <div className="flex justify-between text-xs py-1.5 border-b border-limestone-100">
                      <span className="text-charcoal-700">Ordinary Income (W-2)</span>
                      <span className="font-medium tabular-nums text-charcoal-900">{fmt$(rsuOrdinaryIncome)}</span>
                    </div>
                    <div className="flex justify-between text-xs py-1.5 border-b border-limestone-100">
                      <span className="text-charcoal-700">Federal Withholding ({rsuFedRate}%)</span>
                      <span className="font-medium tabular-nums text-critical-500">-{fmt$(rsuFederalTax)}</span>
                    </div>
                    <div className="flex justify-between text-xs py-1.5 border-b border-limestone-100">
                      <span className="text-charcoal-700">FICA (7.65%)</span>
                      <span className="font-medium tabular-nums text-critical-500">-{fmt$(rsuFICA)}</span>
                    </div>
                    <div className="flex justify-between text-xs py-1.5 border-b border-limestone-100">
                      <span className="text-charcoal-700">State Withholding ({rsuStateRate}%)</span>
                      <span className="font-medium tabular-nums text-critical-500">-{fmt$(rsuStateTax)}</span>
                    </div>
                    <div className="flex justify-between text-xs py-2 border-b-2 border-limestone-200 font-semibold">
                      <span className="text-charcoal-900">Total Tax</span>
                      <span className="tabular-nums text-critical-500">-{fmt$(rsuTotalTax)}</span>
                    </div>
                    <div className="flex justify-between text-xs py-1.5">
                      <span className="text-charcoal-700">Net Shares Delivered</span>
                      <span className="font-medium tabular-nums text-charcoal-900">{rsuNetShares}</span>
                    </div>
                    <div className="flex justify-between text-xs py-1.5">
                      <span className="text-charcoal-700">Net Cash Value</span>
                      <span className="font-medium tabular-nums text-charcoal-900">{fmt$(rsuNetCashValue)}</span>
                    </div>
                  </div>

                  {/* Tax Planning Options */}
                  <div>
                    <h4 className="text-xs font-semibold text-charcoal-900 mb-3">Tax Planning Options</h4>
                        ...
                        <button key={strategy} onClick={() => setRsuTaxStrategy(strategy)}
                          className={`w-full text-left rounded-lg border p-3 text-xs transition-colors ${
                            rsuTaxStrategy === strategy
                              ...
                              : 'border-limestone-200 bg-white text-charcoal-700 hover:border-limestone-300'
                          }`}>
        ...
        {activeTab === 'Stock Options (NQSO / ISO)' && (
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl border border-limestone-200 shadow-sm">
              <div className="px-5 py-4 border-b border-limestone-100 flex items-center justify-between">
                <h3 className="text-sm font-semibold text-charcoal-900">Stock Option Grants</h3>
              ...
              {showAddOption && (
                <div className="px-5 py-4 border-b border-limestone-100 bg-limestone-50">
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    <div>
                      <label className="block text-[10px] font-medium text-charcoal-500 uppercase mb-1">Type</label>
                    ...
                    <div>
                      ...
                      <input type="text" value={newOption.company || ''} onChange={e => setNewOption({ ...newOption, company: e.target.value })}
                        className="w-full rounded-md border border-limestone-300 px-2.5 py-1.5 text-xs focus:border-brand-500 focus:ring-1 focus:ring-brand-500" placeholder="Company" />
                    </div>
                    <div>
                      <label className="block text-[10px] font-medium text-charcoal-500 uppercase mb-1">Ticker</label>
                    ...
                    <div>
                      <label className="block text-[10px] font-medium text-charcoal-500 uppercase mb-1">Exercise Price</label>
                      <input type="number" step="0.01" value={newOption.exercisePrice || ''} onChange={e => setNewOption({ ...newOption, exercisePrice: Number(e.target.value) })}
                        className="w-full rounded-md border border-limestone-300 px-2.5 py-1.5 text-xs focus:border-brand-500 focus:ring-1 focus:ring-brand-500" placeholder="68.50" />
                    </div>
                    <div>
                      <label className="block text-[10px] font-medium text-charcoal-500 uppercase mb-1">Current FMV</label>
                    ...
                    <div>
                      <label className="block text-[10px] font-medium text-charcoal-500 uppercase mb-1">Shares</label>
                      <input type="number" value={newOption.shares || ''} onChange={e => setNewOption({ ...newOption, shares: Number(e.target.value) })}
                        className="w-full rounded-md border border-limestone-300 px-2.5 py-1.5 text-xs focus:border-brand-500 focus:ring-1 focus:ring-brand-500" placeholder="500" />
                    </div>
                    <div>
                      <label className="block text-[10px] font-medium text-charcoal-500 uppercase mb-1">Expiration</label>
                  ...
                  <div className="mt-3 flex justify-end">
                    <button onClick={addOptionGrant}
                      className="rounded-md bg-brand-500 px-4 py-1.5 text-xs font-medium text-white hover:bg-brand-600 transition-colors">
                      Save Option
                    </button>
                  </div>
                </div>
              ...
              {options.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-12 text-center">
                  ...
                  <p className="text-xs text-charcoal-500">No stock options added yet. Click &quot;Add Option Grant&quot; to get started.</p>
                </div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="border-b border-limestone-100 bg-limestone-50">
                        ...
                        <th className="px-4 py-2.5 text-left font-medium text-charcoal-500">Company</th>
                        <th className="px-4 py-2.5 text-left font-medium text-charcoal-500">Grant Date</th>
                        <th className="px-4 py-2.5 text-right font-medium text-charcoal-500">Exercise Price</th>
                        <th className="px-4 py-2.5 text-right font-medium text-charcoal-500">FMV</th>
                        <th className="px-4 py-2.5 text-right font-medium text-charcoal-500">Spread</th>
                        <th className="px-4 py-2.5 text-right font-medium text-charcoal-500">Shares</th>
                        <th className="px-4 py-2.5 text-right font-medium text-charcoal-500">Intrinsic Value</th>
                        <th className="px-4 py-2.5 text-left font-medium text-charcoal-500">Expiration</th>
                        <th className="px-4 py-2.5"></th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-limestone-50">
                      {options.map((o) => {
                        ...
                        return (
                          <tr key={o.id} className={`hover:bg-limestone-50 cursor-pointer ${selectedOption === o.id ? 'bg-brand-50' : ''}`}
                            onClick={() => setSelectedOption(selectedOption === o.id ? null : o.id)}>
                            <td className="px-4 py-2.5">
                              <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-medium ${
                                o.type === 'ISO' ? 'bg-brand-50 text-brand-700' : 'bg-brand-50 text-brand-700'
                              }`}>{o.type}</span>
                            </td>
                            <td className="px-4 py-2.5 font-medium text-charcoal-900">{o.company}</td>
                            <td className="px-4 py-2.5 text-charcoal-700">{o.grantDate}</td>
                            ...
                            <td className="px-4 py-2.5 text-charcoal-700">{o.expiration}</td>
                            <td className="px-4 py-2.5 text-right">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="bg-white rounded-xl border border-limestone-200 shadow-sm">
                <div className="px-5 py-4 border-b border-limestone-100">
                  <h3 className="text-sm font-semibold text-charcoal-900">NQSO Exercise Calculator</h3>
                  <p className="text-[10px] text-charcoal-500 mt-0.5">Spread taxed as ordinary income at exercise</p>
                </div>
                <div className="p-5 space-y-3">
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-[10px] font-medium text-charcoal-500 uppercase mb-1">Shares to Exercise</label>
                  ...
                  {selectedOpt && (
                    <div className="space-y-2 pt-2">
                      <div className="flex justify-between text-xs border-b border-limestone-100 pb-1.5">
                        <span className="text-charcoal-700">Exercise Price</span>
                        <span className="tabular-nums">${selectedOpt.exercisePrice.toFixed(2)}</span>
                      </div>
                      <div className="flex justify-between text-xs border-b border-limestone-100 pb-1.5">
                        <span className="text-charcoal-700">Current FMV</span>
                        <span className="tabular-nums">${selectedOpt.currentFMV.toFixed(2)}</span>
                      </div>
                      <div className="flex justify-between text-xs border-b border-limestone-100 pb-1.5">
                        <span className="text-charcoal-700">Spread / share</span>
                        <span className="tabular-nums text-success-500">${optSpread.toFixed(2)}</span>
                      </div>
                      <div className="flex justify-between text-xs border-b border-limestone-100 pb-1.5">
                        <span className="text-charcoal-700">Gross Ordinary Income</span>
                        <span className="tabular-nums font-medium">{fmt$(optGrossIncome)}</span>
                      </div>
                      <div className="flex justify-between text-xs border-b border-limestone-100 pb-1.5">
                        <span className="text-charcoal-700">Federal Tax ({optFedRate}%)</span>
                      ...
                      <div className="flex justify-between text-xs border-b border-limestone-100 pb-1.5">
                        <span className="text-charcoal-700">State Tax ({optStateRate}%)</span>
                        <span className="tabular-nums text-critical-500">-{fmt$(optStateTaxAmt)}</span>
                      </div>
                      <div className="flex justify-between text-xs font-semibold pt-1">
                        <span className="text-charcoal-900">Net After Tax</span>
                      ...
                      <div className="mt-4 grid grid-cols-2 gap-3">
                        ...
                        <div className="rounded-lg border border-brand-200 bg-brand-50 p-3">
                          <h5 className="text-[10px] font-semibold text-brand-800 uppercase">Exercise + Sell</h5>
                          <p className="text-xs font-medium text-brand-700 mt-1">{fmt$(optNetAfterTax)} net</p>
                          <p className="text-[10px] text-brand-700 mt-0.5">No capital at risk</p>
                          <p className="text-[10px] text-brand-700">All ordinary income</p>
                        </div>
                      </div>
                    </div>
                  )}
              ...
              <div className="bg-white rounded-xl border border-limestone-200 shadow-sm">
                <div className="px-5 py-4 border-b border-limestone-100">
                  <h3 className="text-sm font-semibold text-charcoal-900">ISO Exercise Calculator</h3>
                  <p className="text-[10px] text-charcoal-500 mt-0.5">No ordinary income at exercise; AMT may apply</p>
                </div>
                <div className="p-5 space-y-3">
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-[10px] font-medium text-charcoal-500 uppercase mb-1">AMT Exemption</label>
                  ...
                  {selectedOpt && (
                    <div className="space-y-2 pt-2">
                      <div className="flex justify-between text-xs border-b border-limestone-100 pb-1.5">
                        <span className="text-charcoal-700">Ordinary Income at Exercise</span>
                        <span className="tabular-nums font-medium text-success-500">$0 (ISO)</span>
                      </div>
                      <div className="flex justify-between text-xs border-b border-limestone-100 pb-1.5">
                        <span className="text-charcoal-700">AMT Preference Item</span>
                        <span className="tabular-nums font-medium">{fmt$(isoAMTPreference)}</span>
                      </div>
                      <div className="flex justify-between text-xs border-b border-limestone-100 pb-1.5">
                        <span className="text-charcoal-700">AMT Impact (est. 26%)</span>
                      ...
                      <div className="mt-4 rounded-lg border border-brand-200 bg-brand-50 p-3">
                        <div className="flex items-center gap-1.5 mb-2">
                          ...
                          <h5 className="text-xs font-semibold text-brand-800">ISO AMT Optimizer</h5>
                        </div>
                        <p className="text-[10px] text-brand-700">
                          Maximum ISOs to exercise before triggering AMT:
                        </p>
                        <p className="text-lg font-bold text-brand-900 mt-1">{isoMaxBeforeAMT} shares</p>
        ...
        {activeTab === 'ESPP' && (
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl border border-limestone-200 shadow-sm">
              <div className="px-5 py-4 border-b border-limestone-100 flex items-center justify-between">
                <h3 className="text-sm font-semibold text-charcoal-900">ESPP Purchases</h3>
              ...
              {showAddESPP && (
                <div className="px-5 py-4 border-b border-limestone-100 bg-limestone-50">
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    ...
                    <div>
                      <label className="block text-[10px] font-medium text-charcoal-500 uppercase mb-1">Offering Period</label>
                      <input type="text" value={newESPP.offeringPeriod || ''} onChange={e => setNewESPP({ ...newESPP, offeringPeriod: e.target.value })}
                        className="w-full rounded-md border border-limestone-300 px-2.5 py-1.5 text-xs focus:border-brand-500 focus:ring-1 focus:ring-brand-500" placeholder="Jan 2025 - Jun 2025" />
                    </div>
                    <div>
                      <label className="block text-[10px] font-medium text-charcoal-500 uppercase mb-1">Purchase Date</label>
                    ...
                    <div>
                      ...
                      <input type="number" value={newESPP.shares || ''} onChange={e => setNewESPP({ ...newESPP, shares: Number(e.target.value) })}
                        className="w-full rounded-md border border-limestone-300 px-2.5 py-1.5 text-xs focus:border-brand-500 focus:ring-1 focus:ring-brand-500" placeholder="50" />
                    </div>
                    <div>
                      <label className="block text-[10px] font-medium text-charcoal-500 uppercase mb-1">Purchase Price</label>
                      <input type="number" step="0.01" value={newESPP.purchasePrice || ''} onChange={e => setNewESPP({ ...newESPP, purchasePrice: Number(e.target.value) })}
                        className="w-full rounded-md border border-limestone-300 px-2.5 py-1.5 text-xs focus:border-brand-500 focus:ring-1 focus:ring-brand-500" placeholder="127.50" />
                    </div>
                    <div>
                      <label className="block text-[10px] font-medium text-charcoal-500 uppercase mb-1">FMV at Purchase</label>
                      <input type="number" step="0.01" value={newESPP.fmvAtPurchase || ''} onChange={e => setNewESPP({ ...newESPP, fmvAtPurchase: Number(e.target.value) })}
                        className="w-full rounded-md border border-limestone-300 px-2.5 py-1.5 text-xs focus:border-brand-500 focus:ring-1 focus:ring-brand-500" placeholder="165.00" />
                    </div>
                    <div>
                      <label className="block text-[10px] font-medium text-charcoal-500 uppercase mb-1">Current FMV</label>
              ...
              {esppPurchases.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-12 text-center">
                  ...
                  <p className="text-xs text-charcoal-500">No ESPP purchases added yet. Click &quot;Add ESPP Purchase&quot; to get started.</p>
                </div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="border-b border-limestone-100 bg-limestone-50">
                        <th className="px-4 py-2.5 text-left font-medium text-charcoal-500">Company</th>
                        <th className="px-4 py-2.5 text-left font-medium text-charcoal-500">Offering Period</th>
                        <th className="px-4 py-2.5 text-left font-medium text-charcoal-500">Purchase Date</th>
                        <th className="px-4 py-2.5 text-right font-medium text-charcoal-500">Shares</th>
                        <th className="px-4 py-2.5 text-right font-medium text-charcoal-500">Purchase Price</th>
                        <th className="px-4 py-2.5 text-right font-medium text-charcoal-500">FMV</th>
                        <th className="px-4 py-2.5 text-right font-medium text-charcoal-500">Discount</th>
                        <th className="px-4 py-2.5 text-right font-medium text-charcoal-500">Current Value</th>
                        <th className="px-4 py-2.5"></th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-limestone-50">
                      {esppPurchases.map((p) => (
                        <tr key={p.id} className="hover:bg-limestone-50">
                          <td className="px-4 py-2.5 font-medium text-charcoal-900">{p.company}</td>
                          <td className="px-4 py-2.5 text-charcoal-700">{p.offeringPeriod}</td>
                          <td className="px-4 py-2.5 text-charcoal-700">{p.purchaseDate}</td>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="bg-white rounded-xl border border-limestone-200 shadow-sm">
                <div className="px-5 py-4 border-b border-limestone-100">
                  <h3 className="text-sm font-semibold text-charcoal-900">Disposition Analysis</h3>
                </div>
                <div className="p-5 space-y-3">
                  <div className="flex gap-2 mb-4">
                    <button onClick={() => setEsppQualifying(true)}
                      className={`flex-1 rounded-lg border p-3 text-xs transition-colors ${
                        esppQualifying ? 'border-success-500 bg-success-50 text-success-700' : 'border-limestone-200 text-charcoal-700 hover:border-limestone-300'
                      }`}>
                      <div className="flex items-center gap-1.5 mb-1">
                        <CheckCircle size={14} />
                    ...
                    <button onClick={() => setEsppQualifying(false)}
                      className={`flex-1 rounded-lg border p-3 text-xs transition-colors ${
                        !esppQualifying ? 'border-critical-500 bg-critical-50 text-critical-700' : 'border-limestone-200 text-charcoal-700 hover:border-limestone-300'
                      }`}>
                      <div className="flex items-center gap-1.5 mb-1">
              ...
              <div className="bg-white rounded-xl border border-limestone-200 shadow-sm">
                <div className="px-5 py-4 border-b border-limestone-100">
                  <h3 className="text-sm font-semibold text-charcoal-900">Sale Tax Calculator</h3>
                </div>
                <div className="p-5 space-y-3">
                  <div className="grid grid-cols-2 gap-3 mb-4">
                    <div>
                      <label className="block text-[10px] font-medium text-charcoal-500 uppercase mb-1">Shares to Sell</label>
                      <input type="number" value={esppSaleShares} onChange={e => setEsppSaleShares(Number(e.target.value))}
                        className="w-full rounded-md border border-limestone-300 px-2.5 py-1.5 text-xs focus:border-brand-500 focus:ring-1 focus:ring-brand-500" />
                    </div>
                    <div>
                      <label className="block text-[10px] font-medium text-charcoal-500 uppercase mb-1">Sale Price / Share</label>
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between text-xs border-b border-limestone-100 pb-1.5">
                      <span className="text-charcoal-700">Sale Proceeds</span>
                    ...
                    <div className="flex justify-between text-xs border-b border-limestone-100 pb-1.5">
                      <span className="text-charcoal-700">{esppQualifying ? 'Long-Term' : 'Short-Term'} Capital Gain</span>
                      <span className="tabular-nums text-success-500">{fmt$(Math.max(0, esppCapGain))}</span>
                    </div>
                    <div className="flex justify-between text-xs border-b border-limestone-100 pb-1.5">
                      <span className="text-charcoal-700">Est. Tax on Ordinary</span>
                      <span className="tabular-nums text-critical-500">-{fmt$(esppOrdinaryIncome * 0.32)}</span>
                    </div>
                    <div className="flex justify-between text-xs border-b border-limestone-100 pb-1.5">
                      <span className="text-charcoal-700">Est. Tax on Cap Gain</span>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl border border-limestone-200 shadow-sm">
              <div className="px-5 py-4 border-b border-limestone-100 flex items-center gap-2">
                ...
                <h3 className="text-sm font-semibold text-charcoal-900">Upcoming Vesting Events</h3>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full text-xs">
                  <thead>
                    <tr className="border-b border-limestone-100 bg-limestone-50">
                      <th className="px-4 py-2.5 text-left font-medium text-charcoal-500">Year</th>
                      <th className="px-4 py-2.5 text-left font-medium text-charcoal-500">Quarter</th>
                      <th className="px-4 py-2.5 text-left font-medium text-charcoal-500">Month</th>
                      <th className="px-4 py-2.5 text-left font-medium text-charcoal-500">Grant</th>
                      <th className="px-4 py-2.5 text-right font-medium text-charcoal-500">Shares Vesting</th>
                      <th className="px-4 py-2.5 text-right font-medium text-charcoal-500">Est. Value</th>
                      <th className="px-4 py-2.5 text-right font-medium text-charcoal-500">Est. Tax</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-limestone-50">
                    {vestingEvents.map((ev, idx) => {
                      ...
                      return (
                        <tr key={idx} className={`${isNewQuarter ? 'border-t-2 border-limestone-200' : ''} hover:bg-limestone-50`}>
                          <td className="px-4 py-2.5 font-medium text-charcoal-900">{isNewQuarter ? ev.year : ''}</td>
                          <td className="px-4 py-2.5">
                            {isNewQuarter && (
                          ...
                          <td className="px-4 py-2.5 text-charcoal-700">{ev.month}</td>
                          <td className="px-4 py-2.5 text-charcoal-700">{ev.grant}</td>
                          <td className="px-4 py-2.5 text-right tabular-nums font-medium text-charcoal-900">{ev.shares}</td>
                          <td className="px-4 py-2.5 text-right tabular-nums text-charcoal-900">{fmt$(ev.estValue)}</td>
            ...
            <div className="bg-white rounded-xl border border-limestone-200 shadow-sm">
              <div className="px-5 py-4 border-b border-limestone-100">
                <h3 className="text-sm font-semibold text-charcoal-900">Annual Summary</h3>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full text-xs">
                  <thead>
                    <tr className="border-b border-limestone-100 bg-limestone-50">
                      <th className="px-4 py-2.5 text-left font-medium text-charcoal-500">Year</th>
                      <th className="px-4 py-2.5 text-right font-medium text-charcoal-500">Total Shares Vesting</th>
                      <th className="px-4 py-2.5 text-right font-medium text-charcoal-500">Total Est. Value</th>
                      <th className="px-4 py-2.5 text-right font-medium text-charcoal-500">Total Est. Tax</th>
                      <th className="px-4 py-2.5 text-right font-medium text-charcoal-500">Net After Tax</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-limestone-50">
                    {[2026, 2027].map((year) => {
                      ...
                      return (
                        <tr key={year} className="hover:bg-limestone-50 font-medium">
                          <td className="px-4 py-3 text-charcoal-900">{year}</td>
                          <td className="px-4 py-3 text-right tabular-nums text-charcoal-900">{totalShares}</td>
                          <td className="px-4 py-3 text-right tabular-nums text-charcoal-900">{fmt$(totalValue)}</td>
            ...
            {/* Vesting Bar Chart */}
            <div className="bg-white rounded-xl border border-limestone-200 shadow-sm p-5">
              <h3 className="text-sm font-semibold text-charcoal-900 mb-4">Quarterly Vesting Value</h3>
        ...
        {activeTab === 'Concentrated Position Strategy' && (
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl border border-limestone-200 shadow-sm">
              <div className="px-5 py-4 border-b border-limestone-100">
                <h3 className="text-sm font-semibold text-charcoal-900">Concentrated Position Details</h3>
              </div>
              <div className="p-5">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div>
                    <label className="block text-[10px] font-medium text-charcoal-500 uppercase mb-1">Stock / Asset</label>
                  ...
                  <div>
                    <label className="block text-[10px] font-medium text-charcoal-500 uppercase mb-1">Shares Held</label>
                    <input type="number" value={concPos.sharesHeld} onChange={e => setConcPos({ ...concPos, sharesHeld: Number(e.target.value) })}
                      className="w-full rounded-md border border-limestone-300 px-2.5 py-1.5 text-xs focus:border-brand-500 focus:ring-1 focus:ring-brand-500" />
                  </div>
                  <div>
                    <label className="block text-[10px] font-medium text-charcoal-500 uppercase mb-1">Cost Basis / Share</label>
                    <input type="number" step="0.01" value={concPos.costBasis} onChange={e => setConcPos({ ...concPos, costBasis: Number(e.target.value) })}
                      className="w-full rounded-md border border-limestone-300 px-2.5 py-1.5 text-xs focus:border-brand-500 focus:ring-1 focus:ring-brand-500" />
                  </div>
                  <div>
                    <label className="block text-[10px] font-medium text-charcoal-500 uppercase mb-1">Current Price</label>
                    <input type="number" step="0.01" value={concPos.currentPrice} onChange={e => setConcPos({ ...concPos, currentPrice: Number(e.target.value) })}
                      className="w-full rounded-md border border-limestone-300 px-2.5 py-1.5 text-xs focus:border-brand-500 focus:ring-1 focus:ring-brand-500" />
                  </div>
                  <div>
                    <label className="block text-[10px] font-medium text-charcoal-500 uppercase mb-1">Holding Period</label>
                  ...
                  <div>
                    <label className="block text-[10px] font-medium text-charcoal-500 uppercase mb-1">Total Portfolio Value</label>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-white rounded-xl border border-limestone-200 shadow-sm p-5">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-xs font-medium text-charcoal-500 uppercase">Concentration</span>
                </div>
                <p className="text-2xl font-bold text-warning-500">{fmtPct(concPct)}</p>
                <p className="text-[10px] text-charcoal-500 mt-1">{concPos.ticker} is {fmtPct(concPct)} of portfolio</p>
                {concPct > 10 && (
              ...
              <div className="bg-white rounded-xl border border-limestone-200 shadow-sm p-5">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-xs font-medium text-charcoal-500 uppercase">50% Drop Impact</span>
                </div>
                <p className="text-2xl font-bold text-critical-500">-{fmt$(concDrop50)}</p>
                <p className="text-[10px] text-charcoal-500 mt-1">Portfolio drops to {fmt$(concPos.portfolioValue - concDrop50)}</p>
              </div>
              <div className="bg-white rounded-xl border border-limestone-200 shadow-sm p-5">
                <div className="flex items-center gap-2 mb-2">
                  <ArrowDownRight size={16} className="text-critical-700" />
                  <span className="text-xs font-medium text-charcoal-500 uppercase">75% Drop Impact</span>
                </div>
                <p className="text-2xl font-bold text-critical-700">-{fmt$(concDrop75)}</p>
                <p className="text-[10px] text-charcoal-500 mt-1">Portfolio drops to {fmt$(concPos.portfolioValue - concDrop75)}</p>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl border border-limestone-200 shadow-sm">
              <div className="px-5 py-4 border-b border-limestone-100">
                <h3 className="text-sm font-semibold text-charcoal-900">Diversification Strategies</h3>
              </div>
              <div className="p-5 space-y-4">
                {/* Strategy 1: Outright Sale */}
                <div className="rounded-lg border border-limestone-200 p-4">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <h4 className="text-xs font-semibold text-charcoal-900">Outright Sale</h4>
                  </div>
                  <div className="grid grid-cols-3 gap-4 text-xs">
                    <div>
                      <span className="text-charcoal-500">CG Tax (20% LTCG + 3.8% NIIT)</span>
                      <p className="font-medium text-critical-500 mt-0.5">-{fmt$(concGain * 0.238)}</p>
                    </div>
                    <div>
                      <span className="text-charcoal-500">Net Proceeds</span>
                      <p className="font-medium text-charcoal-900 mt-0.5">{fmt$(concValue - concGain * 0.238)}</p>
                    </div>
                    <div>
                      <span className="text-charcoal-500">Opportunity Cost</span>
                      <p className="font-medium text-charcoal-900 mt-0.5">Immediate diversification</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="rounded-lg border border-limestone-200 p-4">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <h4 className="text-xs font-semibold text-charcoal-900">Exchange Fund</h4>
                  </div>
                  <div className="grid grid-cols-3 gap-4 text-xs">
                    <div>
                      <span className="text-charcoal-500">Tax Deferral</span>
                      <p className="font-medium text-success-500 mt-0.5">100% deferred</p>
                    </div>
                    <div>
                      <span className="text-charcoal-500">Lockup Period</span>
                      <p className="font-medium text-charcoal-900 mt-0.5">7 years</p>
                    </div>
                    <div>
                      <span className="text-charcoal-500">Diversification</span>
                      <p className="font-medium text-charcoal-900 mt-0.5">Receive diversified basket</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="rounded-lg border border-limestone-200 p-4">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <h4 className="text-xs font-semibold text-charcoal-900">Charitable Remainder Trust (CRT)</h4>
                  </div>
                  <div className="grid grid-cols-3 gap-4 text-xs">
                    <div>
                      <span className="text-charcoal-500">Capital Gains Tax</span>
                      <p className="font-medium text-success-500 mt-0.5">$0 (CRT sells tax-free)</p>
                    </div>
                    <div>
                      <span className="text-charcoal-500">Annual Payout (5%)</span>
                      <p className="font-medium text-charcoal-900 mt-0.5">{fmt$(concValue * 0.05)}/yr</p>
                    </div>
                    <div>
                      <span className="text-charcoal-500">Charitable Deduction</span>
                      <p className="font-medium text-charcoal-900 mt-0.5">~{fmt$(concValue * 0.10)} est.</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="rounded-lg border border-limestone-200 p-4">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <h4 className="text-xs font-semibold text-charcoal-900">Equity Collar + Margin Loan</h4>
                  </div>
                  <div className="grid grid-cols-3 gap-4 text-xs">
                    <div>
                      <span className="text-charcoal-500">Hedge Downside</span>
                      <p className="font-medium text-charcoal-900 mt-0.5">Put at 90%, call at 120%</p>
                    </div>
                    <div>
                      <span className="text-charcoal-500">Borrow Against</span>
                      <p className="font-medium text-charcoal-900 mt-0.5">Up to {fmt$(concValue * 0.70)} (70% LTV)</p>
                    </div>
                    <div>
                      <span className="text-charcoal-500">Annual Cost</span>
                ...
                <div className="rounded-lg border border-limestone-200 p-4">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <h4 className="text-xs font-semibold text-charcoal-900">Installment Sale to IDGT</h4>
                  </div>
                  <div className="grid grid-cols-3 gap-4 text-xs">
                    <div>
                      <span className="text-charcoal-500">Transfer to Grantor Trust</span>
                      <p className="font-medium text-charcoal-900 mt-0.5">{fmt$(concValue)} in stock</p>
                    </div>
                    <div>
                      <span className="text-charcoal-500">Installment Note</span>
                      <p className="font-medium text-charcoal-900 mt-0.5">AFR ~5.0%, 9-year note</p>
                    </div>
                    <div>
                      <span className="text-charcoal-500">Estate Impact</span>
            ...
            <div className="bg-white rounded-xl border border-limestone-200 shadow-sm">
              <div className="px-5 py-4 border-b border-limestone-100">
                <h3 className="text-sm font-semibold text-charcoal-900">Strategy Comparison</h3>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full text-xs">
                  <thead>
                    <tr className="border-b border-limestone-100 bg-limestone-50">
                      <th className="px-4 py-2.5 text-left font-medium text-charcoal-500">Strategy</th>
                      <th className="px-4 py-2.5 text-right font-medium text-charcoal-500">Tax Now</th>
                      <th className="px-4 py-2.5 text-right font-medium text-charcoal-500">Tax Deferred</th>
                      <th className="px-4 py-2.5 text-right font-medium text-charcoal-500">Annual Income</th>
                      <th className="px-4 py-2.5 text-left font-medium text-charcoal-500">Estate Impact</th>
                      <th className="px-4 py-2.5 text-left font-medium text-charcoal-500">Complexity</th>
                      <th className="px-4 py-2.5 text-left font-medium text-charcoal-500">Recommended</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-limestone-50">
                    <tr className="hover:bg-limestone-50">
                      <td className="px-4 py-2.5 font-medium text-charcoal-900">Outright Sale</td>
                      <td className="px-4 py-2.5 text-right tabular-nums text-critical-500">{fmt$(concGain * 0.238)}</td>
                      <td className="px-4 py-2.5 text-right tabular-nums text-charcoal-900">$0</td>
                      <td className="px-4 py-2.5 text-right tabular-nums text-charcoal-900">Varies</td>
                      <td className="px-4 py-2.5 text-charcoal-700">Neutral</td>
                    ...
                    <tr className="hover:bg-limestone-50">
                      <td className="px-4 py-2.5 font-medium text-charcoal-900">Exchange Fund</td>
                      <td className="px-4 py-2.5 text-right tabular-nums text-success-500">$0</td>
                      <td className="px-4 py-2.5 text-right tabular-nums text-charcoal-900">{fmt$(concGain * 0.238)}</td>
                      <td className="px-4 py-2.5 text-right tabular-nums text-charcoal-900">$0</td>
                      <td className="px-4 py-2.5 text-charcoal-700">Neutral</td>
                    ...
                    <tr className="hover:bg-limestone-50">
                      <td className="px-4 py-2.5 font-medium text-charcoal-900">CRT</td>
                      <td className="px-4 py-2.5 text-right tabular-nums text-success-500">$0</td>
                      <td className="px-4 py-2.5 text-right tabular-nums text-charcoal-900">Partial</td>
                      <td className="px-4 py-2.5 text-right tabular-nums text-charcoal-900">{fmt$(concValue * 0.05)}</td>
                    ...
                    <tr className="hover:bg-limestone-50">
                      <td className="px-4 py-2.5 font-medium text-charcoal-900">Collar + Margin</td>
                      <td className="px-4 py-2.5 text-right tabular-nums text-success-500">$0</td>
                      <td className="px-4 py-2.5 text-right tabular-nums text-charcoal-900">{fmt$(concGain * 0.238)}</td>
                    ...
                    <tr className="hover:bg-limestone-50">
                      <td className="px-4 py-2.5 font-medium text-charcoal-900">IDGT Sale</td>
                      <td className="px-4 py-2.5 text-right tabular-nums text-success-500">$0</td>
                      <td className="px-4 py-2.5 text-right tabular-nums text-charcoal-900">Deferred</td>
                      <td className="px-4 py-2.5 text-right tabular-nums text-charcoal-900">{fmt$(concValue * 0.05)}</td>
        ...
        {activeTab === 'Exercise / Sale Optimizer' && (
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl border border-limestone-200 shadow-sm">
              <div className="px-5 py-4 border-b border-limestone-100">
                <h3 className="text-sm font-semibold text-charcoal-900">Multi-Year Exercise Plan Optimizer</h3>
                <p className="text-[10px] text-charcoal-500 mt-0.5">Spreading exercises across years to minimize cumulative tax burden</p>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full text-xs">
                  <thead>
                    <tr className="border-b border-limestone-100 bg-limestone-50">
                      <th className="px-4 py-2.5 text-left font-medium text-charcoal-500">Year</th>
                      <th className="px-4 py-2.5 text-right font-medium text-charcoal-500">Projected Income</th>
                      <th className="px-4 py-2.5 text-right font-medium text-charcoal-500">Available Options</th>
                      <th className="px-4 py-2.5 text-right font-medium text-charcoal-500">Recommended Exercise</th>
                      <th className="px-4 py-2.5 text-right font-medium text-charcoal-500">Tax Cost</th>
                      <th className="px-4 py-2.5 text-right font-medium text-charcoal-500">Cumulative Tax</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-limestone-50">
                    {optimizerPlan.map((yr) => (
                      <tr key={yr.year} className="hover:bg-limestone-50">
                        <td className="px-4 py-2.5 font-medium text-charcoal-900">{yr.year}</td>
                        <td className="px-4 py-2.5 text-right tabular-nums text-charcoal-900">{fmt$(yr.projectedIncome)}</td>
                        <td className="px-4 py-2.5 text-right tabular-nums text-charcoal-900">{yr.availableOptions}</td>
                        <td className="px-4 py-2.5 text-right tabular-nums font-medium text-brand-600">{yr.recommendedExercise}</td>
                  ...
                  <tfoot>
                    <tr className="border-t-2 border-limestone-200 bg-limestone-50 font-semibold">
                      <td className="px-4 py-2.5 text-charcoal-900">Total</td>
                      <td className="px-4 py-2.5 text-right tabular-nums text-charcoal-900">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-white rounded-xl border border-limestone-200 shadow-sm">
                <div className="px-5 py-4 border-b border-limestone-100">
                  <h3 className="text-sm font-semibold text-charcoal-900">AMT Breakeven Calculator (ISOs)</h3>
                </div>
                <div className="p-5 space-y-3">
                  <div className="flex justify-between text-xs border-b border-limestone-100 pb-1.5">
                    <span className="text-charcoal-700">ISO Spread / Share</span>
                    <span className="tabular-nums font-medium">${selectedOpt ? optSpread.toFixed(2) : '0.00'}</span>
                  </div>
                  <div className="flex justify-between text-xs border-b border-limestone-100 pb-1.5">
                    <span className="text-charcoal-700">AMT Exemption (MFJ 2025)</span>
                    <span className="tabular-nums font-medium">{fmt$(isoAMTExemption)}</span>
                  </div>
                  <div className="flex justify-between text-xs border-b border-limestone-100 pb-1.5">
                    <span className="text-charcoal-700">Max ISOs Before AMT</span>
                    <span className="tabular-nums font-bold text-brand-600">{isoMaxBeforeAMT} shares</span>
                  </div>
                  <div className="flex justify-between text-xs border-b border-limestone-100 pb-1.5">
                    <span className="text-charcoal-700">Max Spread Before AMT</span>
                    <span className="tabular-nums font-medium">{fmt$(isoMaxBeforeAMT * optSpread)}</span>
                  </div>
                  <div className="flex justify-between text-xs pt-1">
                    <span className="text-charcoal-700">AMT Rate</span>
                    <span className="tabular-nums font-medium">26%</span>
                  </div>

                  <div className="mt-4 rounded-lg bg-brand-50 border border-brand-200 p-3">
              ...
              <div className="bg-white rounded-xl border border-limestone-200 shadow-sm">
                <div className="px-5 py-4 border-b border-limestone-100">
                  <h3 className="text-sm font-semibold text-charcoal-900">Optimal Strategy Recommendation</h3>
                </div>
                <div className="p-5">
                  <div className="rounded-lg bg-brand-50 border border-brand-200 p-4 mb-4">
                    <div className="flex items-center gap-2 mb-2">
                      <CheckCircle size={16} className="text-brand-600" />
                      <h4 className="text-xs font-semibold text-brand-800">Recommended Approach</h4>
                    </div>
