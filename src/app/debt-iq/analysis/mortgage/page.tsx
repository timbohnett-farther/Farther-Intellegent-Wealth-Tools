  ...
  return (
    <div className="rounded-2xl border border-border-subtle bg-surface-soft backdrop-blur-xl p-6 animate-pulse space-y-4">
      {Array.from({ length: rows }).map((_, i) => (
        <div key={i} className="h-4 rounded bg-surface-subtle" style={{ width: `${70 + Math.random() * 30}%` }} />
      ))}
    </div>
  );
}
...
function RefinanceTab({ token }: { token: string | null }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Proposed scenarios */}
      {scenarios.map((s, idx) => (
        <div key={idx} className="rounded-2xl border border-border-subtle bg-surface-soft backdrop-blur-xl p-6">
      ...
      <div className="flex items-center gap-3">
        {scenarios.length < 3 && (
          <button onClick={addScenario} className="rounded-2xl border border-border-subtle bg-surface-soft backdrop-blur-xl px-4 py-2 text-sm font-medium text-text-muted hover:bg-surface-subtle transition-colors">
            + Add Scenario
          </button>
        )}
        <button onClick={runAnalysis} disabled={loading} className="rounded-lg bg-accent-primary px-6 py-2 text-sm font-medium text-text hover:bg-accent-primary/80 disabled:opacity-50 transition-colors shadow-sm">
      ...
      {results && results.length > 0 && (
        <div className="space-y-4">
          <h3 className="text-sm font-semibold uppercase text-text-faint">Results</h3>
          {results.map((r, idx) => (
            <div key={idx} className={`rounded-lg border p-6 ${r.recommendRefinance ? 'border-success-300 bg-success-50/30' : 'border-border-subtle bg-surface-soft'}`}>
              <div className="flex items-center justify-between mb-4">
                <h4 className="text-base font-semibold text-text">{r.scenario}</h4>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                <div>
                  <p className="text-xs text-text-muted">New Payment</p>
                  <p className="text-lg font-bold text-text">{fmtDec.format(r.newMonthlyPayment)}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="text-lg font-bold text-text">{r.willBreakEven ? `${r.breakEvenMonths} months` : 'Never'}</p>
                </div>
                <div>
                  <p className="text-xs text-text-muted">Total Interest Saved</p>
                  <p className="text-lg font-bold text-success-700">{fmt.format(r.totalInterestSavings)}</p>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div className="flex justify-between"><span className="text-text-muted">Current Total Interest</span><span className="font-mono text-text-muted">{fmt.format(r.totalInterestCurrent)}</span></div>
                <div className="flex justify-between"><span className="text-text-muted">New Total Interest</span><span className="font-mono text-text-muted">{fmt.format(r.totalInterestNew)}</span></div>
                <div className="flex justify-between"><span className="text-text-muted">After-Tax NPV</span><span className="font-mono text-text-muted">{fmt.format(r.afterTaxNPV)}</span></div>
                <div className="flex justify-between"><span className="text-text-muted">Net Savings After Costs</span><span className="font-mono text-success-700">{fmt.format(r.netSavingsAfterCosts)}</span></div>
              </div>
              <p className="mt-4 text-sm text-text-muted bg-transparent rounded-lg p-3">{r.recommendation}</p>
            </div>
          ))}
        </div>
      )}
...
function HomeEquityTab({ token }: { token: string | null }) {
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-2xl border border-border-subtle bg-surface-soft backdrop-blur-xl p-6">
        <h3 className="text-sm font-semibold text-text mb-4">Property & Need</h3>
      ...
      {result && (
        <div className="space-y-4">
          <div className="grid grid-cols-3 gap-4">
            <div className="rounded-2xl border border-border-subtle bg-surface-soft backdrop-blur-xl p-4 text-center">
              <p className="text-xs text-text-muted">Current Equity</p>
              <p className="text-xl font-bold text-text">{fmt.format(result.currentEquity)}</p>
            </div>
            <div className="rounded-2xl border border-border-subtle bg-surface-soft backdrop-blur-xl p-4 text-center">
              <p className="text-xs text-text-muted">Current LTV</p>
              <p className="text-xl font-bold text-text">{fmtPct.format(result.currentLTV)}</p>
            </div>
            <div className="rounded-2xl border border-border-subtle bg-surface-soft backdrop-blur-xl p-4 text-center">
              <p className="text-xs text-text-muted">Max Borrowable</p>
              <p className="text-xl font-bold text-text">{fmt.format(result.maxBorrowable)}</p>
            </div>
          </div>
            ...
            <table className="w-full text-sm text-left">
              <thead>
                <tr className="border-b border-limestone-100 bg-transparent/50">
                  <th className="px-4 py-3 text-xs font-semibold uppercase text-text-faint">Feature</th>
                  <th className="px-4 py-3 text-xs font-semibold uppercase text-text-faint">HELOC</th>
                  <th className="px-4 py-3 text-xs font-semibold uppercase text-text-faint">Home Equity Loan</th>
                  <th className="px-4 py-3 text-xs font-semibold uppercase text-text-faint">Cash-Out Refi</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-limestone-100">
                <tr>
                  <td className="px-4 py-3 font-medium text-text-muted">Rate</td>
                ...
                <tr>
                  <td className="px-4 py-3 font-medium text-text-muted">Monthly Payment</td>
                ...
                <tr>
                  <td className="px-4 py-3 font-medium text-text-muted">Total Interest</td>
                ...
                <tr>
                  <td className="px-4 py-3 font-medium text-text-muted">Tax Deductible</td>
                ...
                <tr>
                  <td className="px-4 py-3 font-medium text-text-muted">Closing Costs</td>
...
function MortgageVsInvestTab({ token }: { token: string | null }) {
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-2xl border border-border-subtle bg-surface-soft backdrop-blur-xl p-6">
        <h3 className="text-sm font-semibold text-text mb-4">Inputs</h3>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="mt-4">
          <label className="block text-xs font-medium text-text-muted mb-1">Investment Return: {investReturn}%</label>
          <input type="range" min={2} max={15} step={0.5} value={investReturn} onChange={(e) => setInvestReturn(Number(e.target.value))}
            className="w-full h-2 rounded-full bg-surface-subtle accent-brand-700" />
          <div className="flex justify-between text-xs text-text-faint mt-1"><span>2%</span><span>15%</span></div>
        </div>
        <button onClick={analyze} disabled={loading} className="mt-4 rounded-lg bg-accent-primary px-6 py-2 text-sm font-medium text-text hover:bg-accent-primary/80 disabled:opacity-50 transition-colors shadow-sm">
      ...
      {result && (
        <div className="space-y-4">
          <div className={`rounded-lg border p-4 ${result.recommendation === 'PAY_MORTGAGE' ? 'border-success-300 bg-success-50/30' : result.recommendation === 'INVEST' ? 'border-accent-primarySoft bg-accent-primary/10/30' : 'border-warning-300 bg-warning-50/30'}`}>
            <div className="flex items-center gap-3">
              <span className={`inline-flex rounded-full px-3 py-1 text-xs font-semibold ${result.recommendation === 'PAY_MORTGAGE' ? 'bg-success-100 text-success-700' : result.recommendation === 'INVEST' ? 'bg-accent-primary/15 text-accent-primarySoft' : 'bg-warning-100 text-warning-700'}`}>
                {result.recommendation.replace(/_/g, ' ')}
              </span>
              <span className="text-xs text-text-muted">Confidence: {result.confidenceLevel.replace(/_/g, ' ')}</span>
            </div>
            <p className="text-sm text-text-muted mt-2">{result.narrative}</p>
          </div>
          ...
          <div className="rounded-2xl border border-border-subtle bg-surface-soft backdrop-blur-xl overflow-hidden">
            <table className="w-full text-sm text-left">
              <thead>
                <tr className="border-b border-limestone-100 bg-transparent/50">
                  <th className="px-4 py-3 text-xs font-semibold uppercase text-text-faint">Year</th>
                  <th className="px-4 py-3 text-xs font-semibold uppercase text-text-faint text-right">Pay Mortgage</th>
                  <th className="px-4 py-3 text-xs font-semibold uppercase text-text-faint text-right">Invest Instead</th>
                  <th className="px-4 py-3 text-xs font-semibold uppercase text-text-faint text-right">Difference</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-limestone-100">
                {MILESTONES.map((yr) => {
                  ...
                  return (
                    <tr key={yr}>
                      <td className="px-4 py-3 font-medium text-text-muted">{yr} years</td>
                      <td className="px-4 py-3 text-right font-mono text-text">{fmt.format(mortgageNW)}</td>
                      <td className="px-4 py-3 text-right font-mono text-text">{fmt.format(investNW)}</td>
          ...
          {result.crossoverYear !== null && (
            <p className="text-sm text-text-muted">Crossover year: investing surpasses mortgage payoff at <span className="font-semibold">year {result.crossoverYear}</span>.</p>
          )}
          <p className="text-sm text-text-muted">Break-even investment return needed: <span className="font-semibold">{fmtPct.format(result.breakEvenInvestmentReturn)}</span></p>
        </div>
      )}
    </div>
  );
...
function PMITab({ token }: { token: string | null }) {
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-2xl border border-border-subtle bg-surface-soft backdrop-blur-xl p-6">
        <h3 className="text-sm font-semibold text-text mb-4">Current Mortgage & PMI</h3>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <InputField label="Home Value" value={homeValue} onChange={setHomeValue} prefix="$" />
          <InputField label="Mortgage Balance" value={balance} onChange={setBalance} prefix="$" />
      ...
      {result && (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="rounded-2xl border border-border-subtle bg-surface-soft backdrop-blur-xl p-4">
              <p className="text-xs text-text-muted">Target LTV</p>
              <p className="text-xl font-bold text-text">{fmtPct.format(result.targetLTVForRemoval)}</p>
            </div>
            <div className="rounded-2xl border border-border-subtle bg-surface-soft backdrop-blur-xl p-4">
              <p className="text-xs text-text-muted">Principal Needed</p>
              <p className="text-xl font-bold text-text">{fmt.format(result.principalNeeded)}</p>
            </div>
            <div className="rounded-2xl border border-border-subtle bg-surface-soft backdrop-blur-xl p-4">
              <p className="text-xs text-text-muted">Auto Removal</p>
              <p className="text-xl font-bold text-text">{result.monthsToAutomaticRemoval} months</p>
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-border-subtle bg-surface-soft backdrop-blur-xl p-6">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs font-medium text-text-muted">LTV Progress to 80%</span>
              <span className="text-xs font-semibold text-text-muted">{fmtPct.format(result.currentLTV)}</span>
            </div>
            <div className="h-3 rounded-full bg-surface-subtle overflow-hidden relative">
              <div className="h-full rounded-full bg-accent-primary transition-all" style={{ width: `${Math.min(100, (1 - result.currentLTV) * 500)}%` }} />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="rounded-2xl border border-border-subtle bg-surface-soft backdrop-blur-xl p-6">
              <h4 className="text-sm font-semibold text-text mb-3">Lump Sum Strategy</h4>
              <div className="space-y-2 text-sm">
                <div className="flex justify-between"><span className="text-text-muted">Lump Sum Needed</span><span className="font-mono font-semibold text-text">{fmt.format(result.lumpSumStrategy.lumpSumNeeded)}</span></div>
                <div className="flex justify-between"><span className="text-text-muted">Months Saved</span><span className="font-semibold text-text">{result.lumpSumStrategy.monthsSaved}</span></div>
                <div className="flex justify-between"><span className="text-text-muted">PMI Saved</span><span className="font-semibold text-success-700">{fmt.format(result.lumpSumStrategy.pmiSaved)}</span></div>
                <div className="flex justify-between"><span className="text-text-muted">ROI on Lump Sum</span><span className="font-semibold text-success-700">{fmtPct.format(result.lumpSumStrategy.roiOnLumpSum)}</span></div>
              </div>
            </div>
            <div className="rounded-2xl border border-border-subtle bg-surface-soft backdrop-blur-xl p-6">
              <h4 className="text-sm font-semibold text-text mb-3">Extra Payment Strategy</h4>
              <div className="space-y-2 text-sm">
                <div className="flex justify-between"><span className="text-text-muted">Extra Monthly Needed</span><span className="font-mono font-semibold text-text">{fmtDec.format(result.extraPaymentStrategy.extraMonthlyNeeded)}</span></div>
                <div className="flex justify-between"><span className="text-text-muted">Break-Even</span><span className="font-semibold text-text">{result.extraPaymentStrategy.breakEvenMonths} months</span></div>
                <div className="flex justify-between"><span className="text-text-muted">PMI Savings</span><span className="font-semibold text-success-700">{fmt.format(result.extraPaymentStrategy.pmiSavings)}</span></div>
              </div>
            </div>
          </div>
      ...
      <div className="flex items-center justify-between">
        <div>
          <nav className="flex items-center gap-1 text-sm text-text-faint mb-1">
            ...
            <span>/</span>
            <span className="text-text-muted">Mortgage Analysis</span>
          </nav>
          <h1 className="text-2xl font-bold text-text">Mortgage Analysis</h1>
          <p className="mt-1 text-sm text-text-muted">Refinance, equity access, payoff-vs-invest, and PMI optimization</p>
        </div>
      </div>
