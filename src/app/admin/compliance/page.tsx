    ...
    <div className="space-y-6">
          ...
          return (
            <button
              ...
              className={`inline-flex items-center gap-1.5 px-4 py-2 text-sm font-medium rounded-lg transition-colors ${
                activeSection === s.key ? 'bg-brand-500 text-white' : 'bg-white text-charcoal-500 border border-limestone-200 hover:bg-limestone-50'
              }`}
            >
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-limestone-100 text-left">
                  <th className="px-4 py-3 text-xs font-medium text-charcoal-500 uppercase">Timestamp</th>
                  <th className="px-4 py-3 text-xs font-medium text-charcoal-500 uppercase">Actor</th>
                  <th className="px-4 py-3 text-xs font-medium text-charcoal-500 uppercase">Action</th>
                  <th className="px-4 py-3 text-xs font-medium text-charcoal-500 uppercase">Resource</th>
                  <th className="px-4 py-3 text-xs font-medium text-charcoal-500 uppercase">Detail</th>
                </tr>
              </thead>
              <tbody>
                {filteredAudit.map((e) => (
                  <tr key={e.id} className="border-b border-limestone-50 hover:bg-limestone-50/50">
                    <td className="px-4 py-2.5 text-charcoal-500 whitespace-nowrap">{e.timestamp}</td>
                    <td className="px-4 py-2.5">
                      <span className={`inline-flex items-center gap-1 text-xs font-medium px-2 py-0.5 rounded-full ${
                        e.actorType === 'system' ? 'bg-brand-50 text-brand-700' :
                        e.actorType === 'client' ? 'bg-brand-50 text-brand-700' :
                        'bg-limestone-100 text-charcoal-700'
                      }`}>
                        {e.actor}
                      </span>
                    </td>
                    <td className="px-4 py-2.5">
                      <span className={`text-xs font-medium ${
                        ...
                        e.action === 'UPDATE' ? 'text-brand-700' :
                        'text-charcoal-500'
                      }`}>{e.action}</span>
                    </td>
                    <td className="px-4 py-2.5 text-charcoal-500">{e.resource}</td>
                    <td className="px-4 py-2.5 text-charcoal-500 max-w-xs truncate">{e.detail}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {activeSection === 'regbi' && (
        <div className="bg-white rounded-xl border border-limestone-200 shadow-sm">
          <div className="px-5 py-4 border-b border-limestone-100">
            <h3 className="text-sm font-semibold text-charcoal-900">Reg BI Documentation Tracker</h3>
          </div>
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-limestone-100 text-left">
                <th className="px-5 py-3 text-xs font-medium text-charcoal-500 uppercase">Client</th>
                ...
                <th className="px-5 py-3 text-xs font-medium text-charcoal-500 uppercase text-right">Recommendations</th>
                <th className="px-5 py-3 text-xs font-medium text-charcoal-500 uppercase">Status</th>
                <th className="px-5 py-3 text-xs font-medium text-charcoal-500 uppercase">Client Ack</th>
              </tr>
            </thead>
            <tbody>
              {REG_BI_DOCS.map((doc) => (
                <tr key={doc.id} className="border-b border-limestone-50 hover:bg-limestone-50/50">
                  <td className="px-5 py-3 font-medium text-charcoal-900">{doc.clientName}</td>
                  <td className="px-5 py-3 text-charcoal-500">{doc.advisor}</td>
                  <td className="px-5 py-3 text-charcoal-500">{doc.date}</td>
                  <td className="px-5 py-3 text-charcoal-500 text-right">{doc.recommendations}</td>
      ...
      {activeSection === 'retention' && (
        <div className="space-y-6">
          <div className="grid grid-cols-3 gap-4">
            {[
              ...
              <div key={k.label} className="bg-white rounded-xl border border-limestone-200 shadow-sm p-4">
                ...
                <p className="text-lg font-bold text-charcoal-900">{k.value}</p>
                <p className="text-xs text-charcoal-300 mt-0.5">{k.sub}</p>
              </div>
            ))}
          </div>

          <div className="bg-white rounded-xl border border-limestone-200 shadow-sm p-5">
            <h3 className="text-sm font-semibold text-charcoal-900 mb-3">Retention Policies</h3>
              ...
              {[
                ...
                <div key={p.resource} className="flex items-center justify-between py-2 border-b border-limestone-50 last:border-0">
                  <div className="flex items-center gap-3">
                    <FileText size={14} className="text-charcoal-300" />
                    <span className="text-sm text-charcoal-900">{p.resource}</span>
                  </div>
                  <div className="flex items-center gap-4 text-sm">
                    <span className="text-charcoal-500">{p.retention}</span>
                    <span className="text-charcoal-500">{p.action}</span>
          ...
          <div className="bg-white rounded-xl border border-limestone-200 shadow-sm p-5">
            <h3 className="text-sm font-semibold text-charcoal-900 mb-3">SOC 2 Evidence Collection</h3>
            <div className="grid grid-cols-2 gap-4">
              {[
                ...
                <div key={c.control} className="flex items-start gap-2 p-3 rounded-lg bg-limestone-50">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-charcoal-500">{c.evidence}</p>
                  </div>
                </div>
              ))}
            </div>
