  ...
  return (
    <div className="space-y-6">
          ...
          return (
            <button
              ...
              className={`inline-flex items-center gap-1.5 px-4 py-2 text-sm font-medium rounded-lg transition-colors ${
                activeCategory === c.key ? 'bg-brand-500 text-white' : 'bg-white text-gray-600 border border-gray-200 hover:bg-gray-50'
              }`}
            >
      ...
      {activeCategory === 'assumptions' && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <p className="text-sm text-gray-500">Pre-configured assumption sets for new plans</p>
          ...
          <div className="grid gap-4">
            {ASSUMPTION_TEMPLATES.map((t) => (
              <div key={t.id} className={`bg-white rounded-xl border shadow-sm p-5 ${t.isDefault ? 'border-brand-300' : 'border-gray-200'}`}>
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex items-center gap-1">
                    <button className="p-1.5 text-gray-400 hover:text-gray-600 rounded"><Edit3 size={14} /></button>
                ...
                <div className="grid grid-cols-4 gap-4 mt-3 pt-3 border-t border-gray-100">
                  <div><p className="text-xs text-gray-500">Equity Return</p><p className="text-sm font-medium text-gray-900">{t.equity}</p></div>
                  <div><p className="text-xs text-gray-500">Bond Return</p><p className="text-sm font-medium text-gray-900">{t.bond}</p></div>
                  <div><p className="text-xs text-gray-500">Inflation</p><p className="text-sm font-medium text-gray-900">{t.inflation}</p></div>
                  <div><p className="text-xs text-gray-500">Monte Carlo</p><p className="text-sm font-medium text-gray-900">{t.mc}</p></div>
                </div>
              </div>
            ))}
          </div>
      ...
      {activeCategory === 'reports' && (
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
          <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
            <h3 className="text-sm font-semibold text-gray-900">Report Templates</h3>
          ...
          <table className="w-full text-sm">
            <thead><tr className="border-b border-gray-100 text-left">
              <th className="px-5 py-3 text-xs font-medium text-gray-500 uppercase">Template</th>
              <th className="px-5 py-3 text-xs font-medium text-gray-500 uppercase text-right">Sections</th>
              <th className="px-5 py-3 text-xs font-medium text-gray-500 uppercase text-right">Est. Pages</th>
              <th className="px-5 py-3 text-xs font-medium text-gray-500 uppercase">Last Used</th>
            </tr></thead>
            <tbody>
              {REPORT_TEMPLATES.map((t) => (
                <tr key={t.id} className="border-b border-gray-50 hover:bg-gray-50/50">
                  <td className="px-5 py-3 font-medium text-gray-900">{t.name}</td>
                  <td className="px-5 py-3 text-gray-600 text-right">{t.sections}</td>
                  <td className="px-5 py-3 text-gray-600 text-right">{t.pages}</td>
                  <td className="px-5 py-3 text-gray-500">{t.lastUsed}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {activeCategory === 'emails' && (
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
          ...
          <table className="w-full text-sm">
            <thead><tr className="border-b border-gray-100 text-left">
              <th className="px-5 py-3 text-xs font-medium text-gray-500 uppercase">Template</th>
              <th className="px-5 py-3 text-xs font-medium text-gray-500 uppercase">Subject</th>
              <th className="px-5 py-3 text-xs font-medium text-gray-500 uppercase text-right">Times Used</th>
            </tr></thead>
            <tbody>
              {EMAIL_TEMPLATES.map((t) => (
                <tr key={t.id} className="border-b border-gray-50 hover:bg-gray-50/50">
                  ...
                  <td className="px-5 py-3 text-gray-600 text-right">{t.usage}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {activeCategory === 'workflows' && (
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
          <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
            <h3 className="text-sm font-semibold text-gray-900">Workflow Automation Templates</h3>
          ...
          <table className="w-full text-sm">
            <thead><tr className="border-b border-gray-100 text-left">
              <th className="px-5 py-3 text-xs font-medium text-gray-500 uppercase">Workflow</th>
              <th className="px-5 py-3 text-xs font-medium text-gray-500 uppercase text-right">Steps</th>
              <th className="px-5 py-3 text-xs font-medium text-gray-500 uppercase">Trigger</th>
              <th className="px-5 py-3 text-xs font-medium text-gray-500 uppercase text-right">Executions</th>
            ...
            <tbody>
              {WORKFLOW_TEMPLATES.map((t) => (
                <tr key={t.id} className="border-b border-gray-50 hover:bg-gray-50/50">
                  <td className="px-5 py-3 font-medium text-gray-900">{t.name}</td>
                  <td className="px-5 py-3 text-gray-600 text-right">{t.steps}</td>
                  <td className="px-5 py-3 text-gray-500">{t.trigger}</td>
                  <td className="px-5 py-3 text-gray-600 text-right">{t.executions}</td>
                  <td className="px-5 py-3">
                    <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${t.isSystem ? 'bg-purple-50 text-purple-700' : 'bg-gray-100 text-gray-600'}`}>
