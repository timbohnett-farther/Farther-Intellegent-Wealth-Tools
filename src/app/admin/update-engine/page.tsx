...
const triggerColor: Record<string, string> = {
  quarterly: 'bg-accent-primary/15 text-accent-primarySoft',
  monthly_afr: 'bg-accent-primary/15 text-accent-primarySoft',
  daily_congress: 'bg-surface-subtle text-text-muted',
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-5 gap-4">
        {[
          { label: 'Sources Monitored', value: '57', sub: '6 federal · 51 state', icon: Globe, color: 'text-accent-primarySoft' },
          { label: 'Tables Current', value: `${TABLE_VERSIONS.length}`, sub: 'All validated', icon: Database, color: 'text-success-500' },
          { label: 'Pending Review', value: `${PENDING_REVIEW.length}`, sub: `${PENDING_REVIEW.filter(p => p.severity === 'medium').length} medium priority`, icon: AlertTriangle, color: PENDING_REVIEW.length > 0 ? 'text-warning-500' : 'text-text-faint' },
          { label: 'Last Run', value: '4m 32s', sub: 'Mar 1, 2026 3:00 AM', icon: Clock, color: 'text-accent-primarySoft' },
          { label: 'Plans Recalculated', value: '284', sub: 'Q1 2026 run', icon: Activity, color: 'text-accent-primarySoft' },
        ].map((kpi) => {
          const Icon = kpi.icon;
          return (
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div className="bg-surface-soft rounded-xl border border-border-subtle shadow-sm p-5">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-semibold text-text">Human Review Queue</h3>
            <Link href="/admin/update-engine/review-queue" className="text-xs font-medium text-accent-primarySoft hover:text-accent-primarySoft flex items-center gap-1">
              View All <ChevronRight size={12} />
            </Link>
          </div>
          {PENDING_REVIEW.length === 0 ? (
            <p className="text-sm text-text-faint py-4 text-center">No items pending review</p>
          ) : (
            <div className="space-y-3">
              {PENDING_REVIEW.map((item) => (
                <div key={item.id} className="p-3 rounded-lg border border-limestone-100 hover:border-border-subtle transition-colors">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${
                      ...
                      'bg-accent-primary/15 text-accent-primarySoft'
                    }`}>{item.severity}</span>
                  </div>
                  <p className="text-xs text-text-muted mb-1.5">{item.summary}</p>
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-text-faint">{item.source} · {item.detectedAt}</span>
                    <span className="text-xs text-text-muted">{item.affectedClients.toLocaleString()} clients</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="bg-surface-soft rounded-xl border border-border-subtle shadow-sm p-5">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-semibold text-text">Update Schedule</h3>
            ...
            {SCHEDULE.map((s) => {
              ...
              return (
                <div key={s.name} className="flex items-center gap-3 p-3 rounded-lg bg-transparent">
                  <Icon size={16} className="text-text-faint flex-shrink-0" />
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-text">{s.name}</p>
                    <p className="text-xs text-text-muted">{s.cron}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-xs font-medium text-text-muted">Next run</p>
                    <p className="text-xs text-text-faint">{s.nextRun}</p>
                  </div>
                </div>
              );
            })}
      ...
      {/* Source status */}
      <div className="bg-surface-soft rounded-xl border border-border-subtle shadow-sm p-5">
        <h3 className="text-sm font-semibold text-text mb-3">Data Source Status</h3>
          ...
          {SOURCE_STATUS.map((src) => (
            <div key={src.name} className={`flex items-center gap-2 p-2.5 rounded-lg border ${
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[10px] text-text-muted">{src.lastCheck}</p>
              </div>
              {src.changesLastMonth > 0 && (
                <span className="text-[10px] font-medium text-accent-primarySoft bg-accent-primary/10 px-1.5 py-0.5 rounded-full">
      ...
      <div className="bg-surface-soft rounded-xl border border-border-subtle shadow-sm">
        <div className="px-5 py-4 border-b border-limestone-100">
          <h3 className="text-sm font-semibold text-text">Current Tax Table Versions</h3>
        </div>
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-limestone-100 text-left">
              <th className="px-5 py-3 text-xs font-medium text-text-muted uppercase">Table Type</th>
              <th className="px-5 py-3 text-xs font-medium text-text-muted uppercase">Tax Year</th>
              <th className="px-5 py-3 text-xs font-medium text-text-muted uppercase">Version</th>
              <th className="px-5 py-3 text-xs font-medium text-text-muted uppercase">Last Updated</th>
              <th className="px-5 py-3 text-xs font-medium text-text-muted uppercase">Source</th>
            </tr>
          </thead>
          <tbody>
            {TABLE_VERSIONS.map((tv) => (
              <tr key={tv.type} className="border-b border-limestone-50 hover:bg-surface-subtle/50">
                <td className="px-5 py-3 font-medium text-text">{tv.type}</td>
                <td className="px-5 py-3 text-text-muted">{tv.taxYear}</td>
                ...
                <td className="px-5 py-3 text-text-muted">{tv.lastUpdated}</td>
                <td className="px-5 py-3 text-text-muted text-xs">{tv.source}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div className="bg-surface-soft rounded-xl border border-border-subtle shadow-sm">
        <div className="px-5 py-4 border-b border-limestone-100 flex items-center justify-between">
          ...
          <button onClick={() => setShowAllRuns(!showAllRuns)} className="text-xs font-medium text-accent-primarySoft hover:text-accent-primarySoft">
            ...
            <div key={run.runId} className="px-5 py-4">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  <span className="text-sm font-bold text-text">{run.runId}</span>
                ...
                <span className="text-xs text-text-faint">{run.runDate} · {run.duration}</span>
              </div>
              <p className="text-sm text-text-muted mb-2">{run.summary}</p>
