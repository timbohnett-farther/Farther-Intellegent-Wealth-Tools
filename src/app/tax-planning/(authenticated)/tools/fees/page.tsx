...
function FeeField({
  ...
  return (
    <div>
      ...
      <div className="flex items-center rounded-lg border border-limestone-200 bg-white focus-within:border-brand-700 focus-within:ring-1 focus-within:ring-brand-700 transition-all">
        {prefix && <span className="pl-3 text-sm text-charcoal-500">{prefix}</span>}
        <input
          type="number"
          value={value}
      ...
      {helpText && <p className="mt-1 text-[10px] text-charcoal-400">{helpText}</p>}
    </div>
  );
}
        ...
        return (
          <div key={row.year}>
            <div className="flex items-center justify-between mb-1.5">
              <span className="text-xs font-semibold text-charcoal-700">Year {row.year}</span>
            ...
            <div className="space-y-1">
              <div className="flex items-center gap-2">
                <span className="w-16 text-[10px] text-charcoal-400 text-right">Current</span>
                <div className="flex-1 h-5 rounded bg-limestone-100">
                  <div
                    className="h-full rounded bg-charcoal-300 flex items-center justify-end pr-2 transition-all duration-500"
              ...
              <div className="flex items-center gap-2">
                <span className="w-16 text-[10px] text-brand-600 text-right">Proposed</span>
                <div className="flex-1 h-5 rounded bg-limestone-100">
                  <div
                    className="h-full rounded bg-brand-700 flex items-center justify-end pr-2 transition-all duration-500"
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-charcoal-900">Fee Analyzer</h1>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div className="rounded-lg border border-limestone-200 bg-white p-5 shadow-sm">
          <p className="text-xs font-medium uppercase tracking-wide text-charcoal-500">Current All-In</p>
          <p className="mt-2 text-2xl font-bold text-charcoal-700 tabular-nums">{currentTotalRate.toFixed(2)}%</p>
          <p className="mt-1 text-xs text-charcoal-400">{fmt.format(currentAnnualCost)}/yr</p>
        </div>
        <div className="rounded-lg border border-brand-200 bg-brand-50 p-5 shadow-sm">
          <p className="text-xs font-medium uppercase tracking-wide text-brand-600">Proposed All-In</p>
          <p className="mt-2 text-2xl font-bold text-brand-700 tabular-nums">{proposedTotalRate.toFixed(2)}%</p>
          <p className="mt-1 text-xs text-brand-500">{fmt.format(proposedAnnualCost)}/yr</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="rounded-lg border border-limestone-200 bg-white p-6 shadow-sm space-y-6">
          <h2 className="text-sm font-semibold text-charcoal-900">Fee Inputs</h2>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-medium text-charcoal-700 mb-1.5">Projection Years</label>
              <select
                value={inputs.projectionYears}
                onChange={(e) => updateInput('projectionYears', parseInt(e.target.value))}
                className="h-10 w-full rounded-lg border border-limestone-200 bg-white px-3 text-sm text-charcoal-900 focus:border-brand-700 focus:ring-1 focus:ring-brand-700"
              >
          ...
          {/* Current Fees */}
          <div>
            <h3 className="text-xs font-semibold text-charcoal-700 mb-3 uppercase tracking-wide">Current Fees</h3>
          ...
          {/* Proposed Fees */}
          <div>
            <h3 className="text-xs font-semibold text-brand-700 mb-3 uppercase tracking-wide">Proposed Fees</h3>
            <div className="space-y-3">
              <FeeField
                label="Fund Expense Ratio"
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="rounded-lg border border-limestone-200 bg-white p-6 shadow-sm">
            <h2 className="text-sm font-semibold text-charcoal-900 mb-4">Fee Breakdown Comparison</h2>
            <div className="overflow-x-auto">
              <table className="min-w-full text-sm">
                <thead>
                  <tr className="border-b border-limestone-200">
                    <th className="py-2 text-left text-xs font-semibold uppercase text-charcoal-500">Fee Component</th>
                ...
                <tbody className="divide-y divide-limestone-100">
                  {[
                    ...
                    <tr key={row.label}>
                      <td className="py-2.5 text-charcoal-700">{row.label}</td>
                      <td className="py-2.5 text-right tabular-nums text-charcoal-600">{row.current.toFixed(2)}%</td>
                      <td className="py-2.5 text-right tabular-nums font-medium text-brand-700">{row.proposed.toFixed(2)}%</td>
                ...
                <tfoot className="border-t-2 border-limestone-200">
                  <tr className="font-bold">
                    <td className="py-2.5 text-charcoal-900">Total All-In Cost</td>
                    <td className="py-2.5 text-right tabular-nums text-charcoal-700">{currentTotalRate.toFixed(2)}%</td>
                    <td className="py-2.5 text-right tabular-nums text-brand-700">{proposedTotalRate.toFixed(2)}%</td>
                    <td className="py-2.5 text-right tabular-nums text-success-700">{rateSavings.toFixed(2)}%</td>
                  </tr>
                  <tr>
                    <td className="py-2 text-xs text-charcoal-500">Annual Dollar Cost</td>
                    <td className="py-2 text-right text-xs tabular-nums text-charcoal-500">{fmt.format(currentAnnualCost)}</td>
                    <td className="py-2 text-right text-xs tabular-nums text-brand-600">{fmt.format(proposedAnnualCost)}</td>
          ...
          {/* Projection Table */}
          <div className="rounded-lg border border-limestone-200 bg-white p-6 shadow-sm">
            <h2 className="text-sm font-semibold text-charcoal-900 mb-4">Year-by-Year Projection</h2>
            <div className="overflow-x-auto max-h-64">
              <table className="min-w-full text-sm">
                <thead className="sticky top-0 bg-white">
                  <tr className="border-b border-limestone-200">
                    <th className="py-2 text-left text-xs font-semibold uppercase text-charcoal-500">Year</th>
                    <th className="py-2 text-right text-xs font-semibold uppercase text-charcoal-500">Current</th>
                    <th className="py-2 text-right text-xs font-semibold uppercase text-brand-600">Proposed</th>
                ...
                <tbody className="divide-y divide-limestone-100">
                  {projections.map((row) => (
                    <tr key={row.year} className="hover:bg-limestone-50">
                      <td className="py-1.5 text-charcoal-700 tabular-nums">{row.year}</td>
                      <td className="py-1.5 text-right tabular-nums text-charcoal-600">{fmt.format(row.currentWealth)}</td>
                      <td className="py-1.5 text-right tabular-nums font-medium text-brand-700">{fmt.format(row.proposedWealth)}</td>
