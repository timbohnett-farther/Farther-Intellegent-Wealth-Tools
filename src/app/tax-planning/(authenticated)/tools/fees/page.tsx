...
function FeeField({
  ...
  return (
    <div>
      ...
      <div className="flex items-center rounded-2xl border border-border-subtle bg-surface-soft backdrop-blur-xl focus-within:border-accent-primary focus-within:ring-1 focus-within:ring-accent-primary transition-all">
        {prefix && <span className="pl-3 text-sm text-text-muted">{prefix}</span>}
        <input
          type="number"
          value={value}
          ...
          min={min}
          max={max}
          className="h-10 w-full bg-transparent px-3 text-sm font-mono text-text tabular-nums outline-hidden placeholder:text-text-faint"
        />
        {suffix && <span className="pr-3 text-sm text-text-muted">{suffix}</span>}
      </div>
      {helpText && <p className="mt-1 text-[10px] text-text-faint">{helpText}</p>}
    </div>
  );
}
        ...
        return (
          <div key={row.year}>
            <div className="flex items-center justify-between mb-1.5">
              <span className="text-xs font-semibold text-text-muted">Year {row.year}</span>
            ...
            <div className="space-y-1">
              <div className="flex items-center gap-2">
                <span className="w-16 text-[10px] text-text-faint text-right">Current</span>
                <div className="flex-1 h-5 rounded bg-surface-subtle">
                  <div
                    className="h-full rounded bg-surface-subtle flex items-center justify-end pr-2 transition-all duration-500"
                    style={{ width: `${Math.max(currentPct, 8)}%` }}
                  >
                    <span className="text-[10px] font-bold text-text tabular-nums whitespace-nowrap">
              ...
              <div className="flex items-center gap-2">
                <span className="w-16 text-[10px] text-accent-primarySoft text-right">Proposed</span>
                <div className="flex-1 h-5 rounded bg-surface-subtle">
                  <div
                    className="h-full rounded bg-accent-primary flex items-center justify-end pr-2 transition-all duration-500"
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-text">Fee Analyzer</h1>
        ...
        <div className="flex items-center gap-3">
          <button
            type="button"
            onClick={handleReset}
            className="inline-flex items-center gap-2 rounded-lg border border-border-subtle bg-surface-soft backdrop-blur-xl px-4 py-2.5 text-sm font-medium text-text-muted hover:bg-surface-subtle transition-colors"
          >
          ...
          <Link
            ...
            className="text-sm font-medium text-text-muted hover:text-text-muted transition-colors"
          >
            Back to Proposals
          </Link>
        </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div className="rounded-2xl border border-border-subtle bg-surface-soft backdrop-blur-xl p-5 shadow-sm">
          <p className="text-xs font-medium uppercase tracking-wide text-text-muted">Current All-In</p>
          <p className="mt-2 text-2xl font-bold text-text-muted tabular-nums">{currentTotalRate.toFixed(2)}%</p>
          <p className="mt-1 text-xs text-text-faint">{fmt.format(currentAnnualCost)}/yr</p>
        </div>
        <div className="rounded-lg border border-brand-200 bg-accent-primary/10 p-5 shadow-sm">
          <p className="text-xs font-medium uppercase tracking-wide text-accent-primarySoft">Proposed All-In</p>
          <p className="mt-2 text-2xl font-bold text-accent-primarySoft tabular-nums">{proposedTotalRate.toFixed(2)}%</p>
          <p className="mt-1 text-xs text-accent-primarySoft">{fmt.format(proposedAnnualCost)}/yr</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="rounded-2xl border border-border-subtle bg-surface-soft backdrop-blur-xl p-6 shadow-sm space-y-6">
          <h2 className="text-sm font-semibold text-text">Fee Inputs</h2>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-medium text-text-muted mb-1.5">Projection Years</label>
              <select
                value={inputs.projectionYears}
                onChange={(e) => updateInput('projectionYears', parseInt(e.target.value))}
                className="h-10 w-full rounded-2xl border border-border-subtle bg-surface-soft backdrop-blur-xl px-3 text-sm text-text focus:border-accent-primary focus:ring-1 focus:ring-accent-primary"
              >
          ...
          {/* Current Fees */}
          <div>
            <h3 className="text-xs font-semibold text-text-muted mb-3 uppercase tracking-wide">Current Fees</h3>
          ...
          {/* Proposed Fees */}
          <div>
            <h3 className="text-xs font-semibold text-accent-primarySoft mb-3 uppercase tracking-wide">Proposed Fees</h3>
            <div className="space-y-3">
              <FeeField
                label="Fund Expense Ratio"
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="rounded-2xl border border-border-subtle bg-surface-soft backdrop-blur-xl p-6 shadow-sm">
            <h2 className="text-sm font-semibold text-text mb-4">Fee Breakdown Comparison</h2>
            <div className="overflow-x-auto">
              <table className="min-w-full text-sm">
                <thead>
                  <tr className="border-b border-border-subtle">
                    <th className="py-2 text-left text-xs font-semibold uppercase text-text-muted">Fee Component</th>
                ...
                <tbody className="divide-y divide-limestone-100">
                  {[
                    ...
                    <tr key={row.label}>
                      ...
                      <td className="py-2.5 text-right tabular-nums text-text-muted">{row.current.toFixed(2)}%</td>
                      <td className="py-2.5 text-right tabular-nums font-medium text-accent-primarySoft">{row.proposed.toFixed(2)}%</td>
                ...
                <tfoot className="border-t-2 border-border-subtle">
                  <tr className="font-bold">
                    <td className="py-2.5 text-text">Total All-In Cost</td>
                    <td className="py-2.5 text-right tabular-nums text-text-muted">{currentTotalRate.toFixed(2)}%</td>
                    <td className="py-2.5 text-right tabular-nums text-accent-primarySoft">{proposedTotalRate.toFixed(2)}%</td>
                    <td className="py-2.5 text-right tabular-nums text-success-700">{rateSavings.toFixed(2)}%</td>
                  </tr>
                  <tr>
                    <td className="py-2 text-xs text-text-muted">Annual Dollar Cost</td>
                    <td className="py-2 text-right text-xs tabular-nums text-text-muted">{fmt.format(currentAnnualCost)}</td>
                    <td className="py-2 text-right text-xs tabular-nums text-accent-primarySoft">{fmt.format(proposedAnnualCost)}</td>
          ...
          {/* Projection Table */}
          <div className="rounded-2xl border border-border-subtle bg-surface-soft backdrop-blur-xl p-6 shadow-sm">
            <h2 className="text-sm font-semibold text-text mb-4">Year-by-Year Projection</h2>
            <div className="overflow-x-auto max-h-64">
              <table className="min-w-full text-sm">
                <thead className="sticky top-0 bg-[#3D5A6A]">
                  <tr className="border-b border-border-subtle">
                    <th className="py-2 text-left text-xs font-semibold uppercase text-text-muted">Year</th>
                    <th className="py-2 text-right text-xs font-semibold uppercase text-text-muted">Current</th>
                    <th className="py-2 text-right text-xs font-semibold uppercase text-accent-primarySoft">Proposed</th>
                ...
                <tbody className="divide-y divide-limestone-100">
                  {projections.map((row) => (
                    <tr key={row.year} className="hover:bg-surface-subtle">
                      <td className="py-1.5 text-text-muted tabular-nums">{row.year}</td>
                      <td className="py-1.5 text-right tabular-nums text-text-muted">{fmt.format(row.currentWealth)}</td>
                      <td className="py-1.5 text-right tabular-nums font-medium text-accent-primarySoft">{fmt.format(row.proposedWealth)}</td>
