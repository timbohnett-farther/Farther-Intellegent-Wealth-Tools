    ...
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-charcoal-500">Complete the questionnaire to determine a client&apos;s risk profile and recommended allocation.</p>
        </div>
        <div className="flex items-center gap-3">
          {isComplete && (
            <button type="button" onClick={handleReset} className="inline-flex items-center gap-2 rounded-lg border border-limestone-300 px-4 py-2.5 text-sm font-medium text-charcoal-700 hover:bg-limestone-50">
          ...
          <Link href="/tax-planning/proposals" className="text-sm font-medium text-charcoal-500 hover:text-charcoal-700">Back to Proposals</Link>
        </div>
      </div>
      ...
      <div className="rounded-lg border border-limestone-200 bg-white p-4 shadow-sm">
        ...
        <input
          ...
          className="w-full max-w-md border border-limestone-300 rounded-sm px-3 py-2 text-sm focus:ring-2 focus:ring-brand-400"
        />
      </div>
      ...
      <div className="rounded-lg border border-limestone-200 bg-white p-4 shadow-sm">
        <div className="flex items-center justify-between mb-2">
          ...
          <span className="text-sm font-semibold text-charcoal-900 tabular-nums">{answeredCount}/{totalQuestions} questions</span>
        </div>
        <div className="h-2 w-full rounded-full bg-limestone-100">
          <div className="h-2 rounded-full bg-brand-700 transition-all" style={{ width: `${(answeredCount / totalQuestions) * 100}%` }} />
        </div>
      </div>
          ...
          {!isComplete ? (
            <>
              {RISK_QUESTIONS.map((q, qi) => (
                <div key={q.id} className="rounded-lg border border-limestone-200 bg-white p-5 shadow-sm">
                  <p className="text-sm font-medium text-charcoal-900 mb-3">{qi + 1}. {q.text}</p>
                      ...
                      return (
                        <label
                          ...
                          className={`flex items-center gap-3 rounded-lg border px-4 py-3 cursor-pointer transition-all ${
                            isSelected ? 'border-brand-700 bg-brand-50' : 'border-limestone-200 hover:border-limestone-300 hover:bg-limestone-50'
                          }`}
                        >
                          <input type="radio" name={q.id} checked={isSelected} onChange={() => handleAnswer(q.id, opt.label, opt.score)} className="sr-only" />
                          <div className={`flex h-4 w-4 items-center justify-center rounded-full border-2 ${isSelected ? 'border-brand-700 bg-brand-700' : 'border-limestone-300'}`}>
                            {isSelected && <div className="h-1.5 w-1.5 rounded-full bg-white" />}
                          </div>
                          <span className={`text-sm ${isSelected ? 'text-brand-700 font-medium' : 'text-charcoal-700'}`}>{opt.label}</span>
                        </label>
                      );
                    })}
                  </div>
              ...
              <div className="flex justify-end pt-2">
                <button
                  ...
                  className="inline-flex items-center gap-2 rounded-lg bg-brand-700 px-6 py-2.5 text-sm font-medium text-white hover:bg-brand-600 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  Complete Assessment
                </button>
              </div>
            ...
            <div className="rounded-lg border border-limestone-200 bg-white p-6 shadow-sm space-y-6">
              ...
              <div className="text-center">
                <div className="inline-flex h-24 w-24 items-center justify-center rounded-full bg-brand-50 mb-3">
                  <span className="text-3xl font-bold text-brand-700">{computedScore}</span>
                </div>
                <p className="text-lg font-semibold text-charcoal-900">{riskLabel.replace(/_/g, ' ')}</p>
              </div>

              <div className="grid grid-cols-3 gap-4">
                <div className="rounded-lg bg-limestone-50 p-4 text-center">
                  <p className="text-xs text-charcoal-500">Behavioral</p>
                  <p className="text-xl font-bold text-charcoal-900">{computedScore}</p>
                </div>
                <div className="rounded-lg bg-limestone-50 p-4 text-center">
                  <p className="text-xs text-charcoal-500">Capacity</p>
                  <p className="text-xl font-bold text-charcoal-900">{Math.round(computedScore * 0.9)}</p>
                </div>
                <div className="rounded-lg bg-limestone-50 p-4 text-center">
                  <p className="text-xs text-charcoal-500">Required Return</p>
                  <p className="text-xl font-bold text-charcoal-900">{(4 + (computedScore / 100) * 6).toFixed(1)}%</p>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex h-6 w-full overflow-hidden rounded-full">
                  <div className="bg-brand-700" style={{ width: `${allocation.equity}%` }} />
                  ...
                  <div className="bg-limestone-300" style={{ width: `${allocation.cash}%` }} />
                </div>
                <div className="mt-2 flex flex-wrap gap-4 text-xs text-charcoal-500">
                  <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-brand-700" />Equity {allocation.equity}%</span>
                  ...
                  <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-limestone-300" />Cash {allocation.cash}%</span>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3 pt-4 border-t border-limestone-100">
                <button
                  ...
                  className="inline-flex items-center gap-2 rounded-lg bg-brand-700 px-5 py-2.5 text-sm font-medium text-white hover:bg-brand-600"
                >
                  Use in Proposal
                </button>
                <button
                  ...
                  disabled={saving}
                  className="inline-flex items-center gap-2 rounded-lg border border-limestone-300 px-5 py-2.5 text-sm font-medium text-charcoal-700 hover:bg-limestone-50 disabled:opacity-50"
                >
                  {saving ? 'Saving...' : 'Save Profile'}
                </button>
                <button type="button" onClick={handleReset} className="text-sm font-medium text-charcoal-500 hover:text-charcoal-700">Start Over</button>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-lg border border-limestone-200 bg-white p-5 shadow-sm sticky top-4">
            ...
            <div className="text-center mb-4">
              <div className="inline-flex h-16 w-16 items-center justify-center rounded-full bg-brand-50">
                <span className="text-2xl font-bold text-brand-700">{computedScore}</span>
              </div>
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between"><span className="text-charcoal-500">Questions Answered</span><span className="font-medium text-charcoal-900 tabular-nums">{answeredCount}/{totalQuestions}</span></div>
            </div>
            <div className="mt-4 pt-4 border-t border-limestone-100">
              <p className="text-[10px] font-medium uppercase text-charcoal-400 mb-2">Suggested Allocation</p>
              ...
              <div className="mt-2 grid grid-cols-2 gap-1 text-[10px]">
                <div className="flex justify-between"><span className="text-charcoal-400">Equity</span><span className="font-semibold text-charcoal-700">{allocation.equity}%</span></div>
                <div className="flex justify-between"><span className="text-charcoal-400">Fixed Inc</span><span className="font-semibold text-charcoal-700">{allocation.fixedIncome}%</span></div>
                <div className="flex justify-between"><span className="text-charcoal-400">Alts</span><span className="font-semibold text-charcoal-700">{allocation.alternatives}%</span></div>
                <div className="flex justify-between"><span className="text-charcoal-400">Cash</span><span className="font-semibold text-charcoal-700">{allocation.cash}%</span></div>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-lg border border-limestone-200 bg-white p-5 shadow-sm">
            <h3 className="text-sm font-semibold text-charcoal-900 mb-3">Past Assessments</h3>
            {loadingHistory ? (
              <div className="space-y-2 animate-pulse">
                {[1, 2, 3].map((i) => <div key={i} className="h-10 rounded bg-limestone-200" />)}
              </div>
            ) : pastAssessments.length === 0 ? (
              <p className="text-xs text-charcoal-500">No past assessments found.</p>
            ) : (
              <div className="space-y-2 max-h-48 overflow-y-auto">
                {pastAssessments.map((a) => (
                  <div key={a.assessmentId} className="flex items-center justify-between rounded-md bg-limestone-50 px-3 py-2">
                    <div>
                      <p className="text-xs font-medium text-charcoal-900">{a.clientName}</p>
                      <p className="text-[10px] text-charcoal-500">{formatDate(a.createdAt)}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-sm font-bold text-charcoal-900">{a.compositeScore}</p>
