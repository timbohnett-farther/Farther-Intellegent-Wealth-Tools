  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-lg border border-limestone-200 bg-white p-6">
        <h3 className="text-sm font-semibold text-charcoal-900 mb-4">Portfolio & Loan Details</h3>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="rounded-lg border border-limestone-200 bg-white p-4">
              <p className="text-xs text-charcoal-500">Max Borrowing Power</p>
              <p className="text-xl font-bold text-charcoal-900">{fmt.format(result.maxBorrowingPower)}</p>
            </div>
            <div className="rounded-lg border border-limestone-200 bg-white p-4">
              <p className="text-xs text-charcoal-500">Loan-to-Value</p>
              <p className="text-xl font-bold text-charcoal-900">{fmtPct.format(result.loanToValue)}</p>
            </div>
            <div className="rounded-lg border border-limestone-200 bg-white p-4">
              <p className="text-xs text-charcoal-500">LTV Rating</p>
              <div className="mt-1"><LTVBadge rating={result.ltvRating} /></div>
            </div>
            <div className="rounded-lg border border-limestone-200 bg-white p-4">
              <p className="text-xs text-charcoal-500">Annual Interest Cost</p>
              <p className="text-xl font-bold text-charcoal-900">{fmt.format(result.costAnalysis.annualInterestCost)}</p>
            </div>
          </div>

          {/* Margin call analysis */}
          <div className="rounded-lg border border-limestone-200 bg-white p-6">
            <h3 className="text-sm font-semibold text-charcoal-900 mb-4">Margin Call Analysis</h3>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
              <div>
                <p className="text-xs text-charcoal-500">Call Threshold</p>
                <p className="text-lg font-bold text-charcoal-900">{fmtPct.format(result.marginCallAnalysis.marginCallThreshold)}</p>
              </div>
              <div>
                <p className="text-xs text-charcoal-500">Buffer Amount</p>
                <p className="text-lg font-bold text-charcoal-900">{fmt.format(result.marginCallAnalysis.bufferAmount)}</p>
              </div>
              <div>
                <p className="text-xs text-charcoal-500">Portfolio Drop to Call</p>
              ...
              <div>
                <p className="text-xs text-charcoal-500">Maintenance Call</p>
                <p className="text-lg font-bold text-charcoal-900">{fmt.format(result.marginCallAnalysis.maintenanceCallAmount)}</p>
              </div>
            </div>

            {/* Probability estimates */}
            <div className="rounded-lg bg-limestone-50 p-4">
              <p className="text-xs font-semibold uppercase text-charcoal-400 mb-3">Margin Call Probability</p>
              <div className="grid grid-cols-3 gap-4 text-sm">
                <div className="text-center">
                  <p className="text-xs text-charcoal-500 mb-1">Next 6 Months</p>
                ...
                <div className="text-center">
                  <p className="text-xs text-charcoal-500 mb-1">Next 12 Months</p>
                ...
                <div className="text-center">
                  <p className="text-xs text-charcoal-500 mb-1">Next Bear Market</p>
          ...
          {/* Cost comparison */}
          <div className="rounded-lg border border-limestone-200 bg-white p-6">
            <h3 className="text-sm font-semibold text-charcoal-900 mb-4">Cost Comparison</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {/* vs Liquidating */}
              <div className="rounded-lg bg-limestone-50 p-4">
                <p className="text-xs font-semibold uppercase text-charcoal-400 mb-3">vs. Liquidating Portfolio</p>
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    <span className="text-charcoal-500">Capital Gains Tax Avoided</span>
                  ...
                  <div className="flex justify-between">
                    <span className="text-charcoal-500">Portfolio Growth Lost</span>
                  ...
                  <div className="flex justify-between border-t border-limestone-200 pt-2">
                    <span className="text-charcoal-700 font-medium">Net Advantage of SBLOC</span>
              ...
              {/* vs Mortgage */}
              <div className="rounded-lg bg-limestone-50 p-4">
                <p className="text-xs font-semibold uppercase text-charcoal-400 mb-3">vs. Mortgage</p>
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    <span className="text-charcoal-500">Mortgage Interest</span>
                    <span className="font-mono text-charcoal-700">{fmt.format(result.costAnalysis.vsMortgage.mortgageInterest)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-charcoal-500">SBLOC Interest</span>
                    <span className="font-mono text-charcoal-700">{fmt.format(result.costAnalysis.vsMortgage.sblocInterest)}</span>
                  </div>
                  <div className="flex justify-between border-t border-limestone-200 pt-2">
                    <span className="text-charcoal-700 font-medium">Net Difference</span>
                    <span className="font-mono font-semibold text-charcoal-900">{fmt.format(result.costAnalysis.vsMortgage.netDifference)}</span>
                  </div>
                  <p className="text-xs text-charcoal-500">{result.costAnalysis.vsMortgage.speedAdvantage}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          {result.rateStressTest && result.rateStressTest.length > 0 && (
            <div className="rounded-lg border border-limestone-200 bg-white overflow-hidden">
              <div className="px-6 py-4 border-b border-limestone-100">
                <h3 className="text-sm font-semibold text-charcoal-900">Rate Stress Test</h3>
              </div>
              <table className="w-full text-sm text-left">
                <thead>
                  <tr className="border-b border-limestone-100 bg-limestone-50/50">
                    <th className="px-4 py-3 text-xs font-semibold uppercase text-charcoal-400">Scenario</th>
                    <th className="px-4 py-3 text-xs font-semibold uppercase text-charcoal-400 text-right">New Monthly Payment</th>
                    <th className="px-4 py-3 text-xs font-semibold uppercase text-charcoal-400 text-right">Annual Cost Increase</th>
                ...
                <tbody className="divide-y divide-limestone-100">
                  {result.rateStressTest.map((test, i) => (
                    <tr key={i} className="hover:bg-limestone-50/50">
                      <td className="px-4 py-3 font-medium text-charcoal-900">{test.rateScenario}</td>
                      <td className="px-4 py-3 text-right font-mono text-charcoal-700">{fmtDec.format(test.newMonthlyPayment)}</td>
                      <td className="px-4 py-3 text-right font-mono text-critical-700">{fmt.format(test.annualCostIncrease)}</td>
                      <td className="px-4 py-3 text-charcoal-600">{test.recommendation}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      {!loading && !result && (
        <div className="rounded-lg border border-limestone-200 bg-white p-12 text-center">
          <p className="text-sm text-charcoal-500">Enter your portfolio and loan details to analyze a Securities-Backed Line of Credit.</p>
        </div>
      )}
    </div>
  );
...
function MarginLoanTab({ token }: { token: string | null }) {
  ...
  return (
    <div className="space-y-6">
      {/* Inputs */}
      <div className="rounded-lg border border-limestone-200 bg-white p-6">
        <h3 className="text-sm font-semibold text-charcoal-900 mb-4">Margin Loan Details</h3>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          <InputField label="Portfolio Value" value={portfolioValue} onChange={setPortfolioValue} prefix="$" />
      ...
      {result && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="rounded-lg border border-limestone-200 bg-white p-4">
              <p className="text-xs text-charcoal-500">Reg T Initial Margin</p>
              <p className="text-xl font-bold text-charcoal-900">{fmt.format(result.initialMarginRequired)}</p>
            </div>
            <div className="rounded-lg border border-limestone-200 bg-white p-4">
              <p className="text-xs text-charcoal-500">Maintenance Margin</p>
              <p className="text-xl font-bold text-charcoal-900">{fmtPct.format(result.maintenanceMarginLevel)}</p>
            </div>
            <div className="rounded-lg border border-limestone-200 bg-white p-4">
            ...
            <div className="rounded-lg border border-limestone-200 bg-white p-4">
              <p className="text-xs text-charcoal-500">Equity Buffer</p>
          ...
          {/* Risk metrics */}
          <div className="rounded-lg border border-limestone-200 bg-white p-6">
            <h3 className="text-sm font-semibold text-charcoal-900 mb-4">Risk Metrics</h3>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div>
                <p className="text-xs text-charcoal-500">Drop to Margin Call</p>
              ...
              <div>
                <p className="text-xs text-charcoal-500">Daily VaR (95%)</p>
                <p className="text-lg font-bold text-charcoal-900">{fmt.format(result.dailyVaR)}</p>
              </div>
              <div>
              ...
              <div>
                <p className="text-xs text-charcoal-500">Annual Interest Cost</p>
                <p className="text-lg font-bold text-charcoal-900">{fmt.format(result.annualInterestCost)}</p>
              </div>
            </div>
          </div>
      ...
      {!loading && !result && (
        <div className="rounded-lg border border-limestone-200 bg-white p-12 text-center">
          <p className="text-sm text-charcoal-500">Enter your portfolio and margin loan details to analyze risk exposure.</p>
        </div>
      )}
    </div>
  );
    ...
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      <div>
        <nav className="flex items-center gap-1 text-sm text-charcoal-400 mb-1">
          ...
          <span>/</span>
          <span className="text-charcoal-700">Securities-Backed Lending</span>
        </nav>
        <h1 className="text-2xl font-bold text-charcoal-900">Securities-Backed Lending</h1>
        <p className="mt-1 text-sm text-charcoal-500">SBLOC analysis, margin call risk, and cost comparison tools</p>
      </div>
