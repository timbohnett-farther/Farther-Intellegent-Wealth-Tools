...
function MetricGauge({ label, value, format, thresholds }: {
  ...
  return (
    <div className="rounded-lg border border-limestone-200 bg-white p-4">
      ...
      <p className="text-xl font-bold text-charcoal-900 mb-2">{format(value)}</p>
      <div className="h-2 rounded-full bg-limestone-100 overflow-hidden">
    ...
    <div className="mx-auto max-w-6xl space-y-8">
      ...
      <div>
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="text-sm text-charcoal-500">Business Debt Analyzer</span>
        </div>
        <h1 className="text-2xl font-bold text-charcoal-900">Business Debt Analyzer</h1>
        <p className="mt-1 text-sm text-charcoal-500">Leverage metrics, personal exposure, and Section 163(j) analysis</p>
      </div>

      {/* Business Info */}
      <div className="rounded-xl border border-limestone-200 bg-white p-6 shadow-sm">
        <h3 className="text-sm font-semibold text-charcoal-900 mb-4">Business Profile</h3>
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          <div>
            <label className="block text-xs font-medium text-charcoal-500 mb-1">Entity Type</label>
            <select
              value={businessType}
      ...
      <div className="rounded-xl border border-limestone-200 bg-white shadow-sm overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-limestone-100">
          <h3 className="text-sm font-semibold text-charcoal-900">Business Debts</h3>
          <button onClick={addDebt} className="text-sm font-medium text-brand-700 hover:text-brand-600">+ Add Debt</button>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-limestone-100 bg-limestone-50/50">
                <th className="px-4 py-3 text-left text-xs font-semibold uppercase text-charcoal-400">Type</th>
                <th className="px-4 py-3 text-right text-xs font-semibold uppercase text-charcoal-400">Balance</th>
                <th className="px-4 py-3 text-right text-xs font-semibold uppercase text-charcoal-400">Rate</th>
                <th className="px-4 py-3 text-right text-xs font-semibold uppercase text-charcoal-400">Term (mo)</th>
                <th className="px-4 py-3 text-center text-xs font-semibold uppercase text-charcoal-400">PG</th>
                <th className="px-4 py-3 text-center text-xs font-semibold uppercase text-charcoal-400">163(j)</th>
                <th className="px-4 py-3 text-xs font-semibold uppercase text-charcoal-400" />
              </tr>
            </thead>
      ...
      {result && (
        <>
          {/* Leverage Metrics */}
          <div>
            <h3 className="text-sm font-semibold uppercase text-charcoal-400 mb-3">Business Leverage Metrics</h3>
          ...
          <div className="rounded-xl border border-limestone-200 bg-white p-6 shadow-sm">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-semibold uppercase text-charcoal-400">Personal Exposure</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="flex justify-between text-sm py-2 border-b border-limestone-100">
                <span className="text-charcoal-500">Personally Guaranteed</span>
                <span className="font-mono font-semibold text-charcoal-900">{fmt.format(result.personalExposure.personallyGuaranteed)}</span>
              </div>
              <div className="flex justify-between text-sm py-2 border-b border-limestone-100">
                <span className="text-charcoal-500">% of Net Worth</span>
                <span className="font-mono font-semibold text-charcoal-900">{fmtPct.format(result.personalExposure.percentOfNetWorth)}</span>
              </div>
            </div>
          </div>
          ...
          {/* Tax Deductibility */}
          <div className="rounded-xl border border-limestone-200 bg-white p-6 shadow-sm">
            <h3 className="text-sm font-semibold uppercase text-charcoal-400 mb-4">Section 163(j) Interest Deduction</h3>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div>
                <p className="text-xs text-charcoal-500 mb-1">163(j) Limit (30% EBITDA)</p>
                <p className="text-lg font-bold text-charcoal-900">{fmt.format(result.taxDeductibility.section163jLimit)}</p>
              </div>
              <div>
                <p className="text-xs text-charcoal-500 mb-1">Deductible This Year</p>
                <p className="text-lg font-bold text-success-700">{fmt.format(result.taxDeductibility.deductibleThisYear)}</p>
              </div>
              <div>
                <p className="text-xs text-charcoal-500 mb-1">Disallowed Carryforward</p>
                <p className="text-lg font-bold text-warning-700">{fmt.format(result.taxDeductibility.disallowedCarryforward)}</p>
              </div>
              <div>
                <p className="text-xs text-charcoal-500 mb-1">Tax Savings</p>
          ...
          {result.recommendations.length > 0 && (
            <div className="rounded-xl border border-brand-200 bg-brand-50 p-6">
              <h3 className="text-sm font-semibold text-brand-700 mb-3">Recommendations</h3>
