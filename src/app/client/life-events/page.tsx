...
function statusConfig(status: string) {
  switch (status) {
    ...
    case 'applied':
      return { label: 'Applied to Plan', icon: Check, className: 'bg-teal-500/10 text-teal-300' };
    default:
      return { label: status, icon: AlertCircle, className: 'bg-white/[0.06] text-white/50' };
  }
}
...
function EventWizard({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 overflow-y-auto py-8">
      <div className="bg-white/[0.07] rounded-modal shadow-xl w-full max-w-lg mx-4">
        {/* Header */}
        ...
        <form onSubmit={handleSubmit} className="px-6 py-4 space-y-4">
            ...
            <div key={field.name}>
              ...
              {field.type === 'textarea' ? (
                <textarea
                  ...
                  className="w-full border border-white/[0.10] rounded-input px-3 py-2 text-sm focus:outline-hidden focus:ring-2 focus:ring-teal-500"
                />
              ) : field.type === 'select' ? (
                <select
                  value={formData[field.name] || ''}
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-limestone-100">
            ...
            <button
              type="submit"
              className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-teal-500 rounded-lg hover:bg-teal-400"
            >
              <Send size={16} />
              Submit to Advisor
            </button>
        ...
        {LIFE_EVENTS.map((event) => {
          ...
          return (
            <button
              ...
              className="bg-white/[0.07] rounded-card border border-white/[0.06] p-5 text-left hover:border-teal-300 hover:shadow-sm transition-all group"
            >
              <div className="flex items-start gap-3">
                ...
                <ChevronRight
                  size={16}
                  className="text-white/30 group-hover:text-teal-300 mt-1 flex-shrink-0"
                />
              </div>
            </button>
          );
                    ...
                    <div className="flex items-center gap-3">
                      <div
                        className={`w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 ${
                          eventDef?.color || 'bg-white/[0.06] text-white/50'
                        }`}
                      >
                        <EventIcon size={18} />
                      </div>
