    ...
    <div className="space-y-4">
        ...
        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg p-4">
          <div className="grid grid-cols-2 gap-4 text-center">
            <div className="bg-white rounded-lg p-3 shadow-sm border border-blue-100">
              <p className="text-xs text-gray-500 mb-1">Bull Call Spread</p>
              <div className="text-sm">
                <p className="text-green-600 font-medium">Buy Call K1</p>
                <p className="text-red-600 font-medium">Sell Call K2</p>
              </div>
            </div>
            <div className="bg-white rounded-lg p-3 shadow-sm border border-blue-100">
              <p className="text-xs text-gray-500 mb-1">Bear Put Spread</p>
              <div className="text-sm">
                <p className="text-green-600 font-medium">Buy Put K2</p>
                <p className="text-red-600 font-medium">Sell Put K1</p>
              </div>
            </div>
          </div>
          <div className="mt-3 text-center">
            <p className="text-xs text-gray-500">Strike Width: {data.strikeWidth} points | Multiplier: ${data.multiplier} | Contracts: {data.contracts}</p>
          </div>
        </div>
      </div>
      ...
      <div className="card overflow-hidden">
        ...
        <div className="divide-y divide-gray-100">
          ...
          <div className="flex justify-between px-4 py-3 text-sm bg-blue-50 font-semibold">
            <span className="text-blue-800">All-In Borrowing Rate</span>
            <span className="text-blue-800">{formatPercent(data.allInRate)}</span>
          </div>
        </div>
      </div>
      ...
      <div className="card p-4">
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="text-center p-3 bg-green-50 rounded-lg">
            <p className="text-xs text-gray-500 mb-1">Proceeds Received</p>
            <p className="text-lg font-bold text-green-700">{formatCurrency(data.actualProceeds)}</p>
          </div>
          <div className="text-center p-3 bg-red-50 rounded-lg">
            <p className="text-xs text-gray-500 mb-1">Repayment Due</p>
            <p className="text-lg font-bold text-red-700">{formatCurrency(data.actualRepayment)}</p>
          </div>
          <div className="text-center p-3 bg-gray-50 rounded-lg">
            <p className="text-xs text-gray-500 mb-1">Interest Paid</p>
            <p className="text-lg font-bold text-gray-700">{formatCurrency(data.interestPaid)}</p>
          </div>
        </div>
