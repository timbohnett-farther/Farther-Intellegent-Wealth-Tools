...
const TAX_BUCKET_CONFIG: Record<TaxBucket, { label: string; variant: 'warning' | 'purple' | 'info' | 'default' }> = {
  taxable: { label: 'Taxable', variant: 'warning' },
  tax_deferred: { label: 'Tax-Deferred', variant: 'purple' },
  tax_free: { label: 'Tax-Free', variant: 'info' },
  non_financial: { label: 'Non-Financial', variant: 'default' },
};
...
function AccountIcon({ accountType }: { accountType: string }) {
  ...
  if (t.includes('checking') || t.includes('savings') || t.includes('money_market') || t.includes('cd'))
    return <Landmark className="h-4 w-4 text-gray-400" aria-hidden="true" />;
  if (t.includes('brokerage') || t.includes('crypto') || t.includes('hedge') || t.includes('private'))
    return <Wallet className="h-4 w-4 text-gray-400" aria-hidden="true" />;
  if (t.includes('ira') || t.includes('401k') || t.includes('403b') || t.includes('457') || t.includes('pension') || t.includes('roth'))
    return <PiggyBank className="h-4 w-4 text-gray-400" aria-hidden="true" />;
  if (t.includes('insurance') || t.includes('hsa') || t.includes('529'))
    return <ShieldCheck className="h-4 w-4 text-gray-400" aria-hidden="true" />;
  if (t.includes('real_estate') || t.includes('mortgage') || t.includes('heloc'))
    return <Home className="h-4 w-4 text-gray-400" aria-hidden="true" />;
  if (t.includes('credit_card') || t.includes('loan') || t.includes('sbloc') || t.includes('margin'))
    return <CreditCard className="h-4 w-4 text-gray-400" aria-hidden="true" />;
  return <Wallet className="h-4 w-4 text-gray-400" aria-hidden="true" />;
}
...
export function AccountRow({
  ...
  return (
    <div
      onClick={onClick}
      className={clsx(
        'flex items-center gap-4 rounded-lg border border-gray-200 bg-white px-4 py-3 transition-colors',
    ...
    >
      ...
      <p
        className={clsx(
          ...
          balance < 0 ? 'text-[#EF4444]' : 'text-gray-900',
        )}
      >
      ...
      <div className="hidden sm:flex flex-col items-end shrink-0 w-24">
        <span className="text-[10px] text-gray-400 truncate">{dataSource}</span>
        {lastSync && (
