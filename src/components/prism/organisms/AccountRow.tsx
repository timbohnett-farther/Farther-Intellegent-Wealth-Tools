...
function AccountIcon({ accountType }: { accountType: string }) {
  ...
  if (t.includes('checking') || t.includes('savings') || t.includes('money_market') || t.includes('cd'))
    return <Landmark className="h-4 w-4 text-charcoal-300" aria-hidden="true" />;
  if (t.includes('brokerage') || t.includes('crypto') || t.includes('hedge') || t.includes('private'))
    return <Wallet className="h-4 w-4 text-charcoal-300" aria-hidden="true" />;
  if (t.includes('ira') || t.includes('401k') || t.includes('403b') || t.includes('457') || t.includes('pension') || t.includes('roth'))
    return <PiggyBank className="h-4 w-4 text-charcoal-300" aria-hidden="true" />;
  if (t.includes('insurance') || t.includes('hsa') || t.includes('529'))
    return <ShieldCheck className="h-4 w-4 text-charcoal-300" aria-hidden="true" />;
  if (t.includes('real_estate') || t.includes('mortgage') || t.includes('heloc'))
    return <Home className="h-4 w-4 text-charcoal-300" aria-hidden="true" />;
  if (t.includes('credit_card') || t.includes('loan') || t.includes('sbloc') || t.includes('margin'))
    return <CreditCard className="h-4 w-4 text-charcoal-300" aria-hidden="true" />;
  return <Wallet className="h-4 w-4 text-charcoal-300" aria-hidden="true" />;
}
...
export function AccountRow({
  ...
  return (
    ...
    >
      {/* Icon */}
      <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-limestone-100">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="mt-0.5 flex items-center gap-2 text-xs text-charcoal-300">
      ...
      <div className="hidden sm:flex flex-col items-end shrink-0 w-24">
        <span className="text-[10px] text-charcoal-300 truncate">{dataSource}</span>
        {lastSync && (
          <span className="flex items-center gap-1 text-[10px] text-charcoal-300">
