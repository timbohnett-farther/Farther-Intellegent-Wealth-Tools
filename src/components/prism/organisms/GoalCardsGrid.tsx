...
const STATUS_CONFIG: Record<GoalStatus, { label: string; bg: string; text: string }> = {
  ...
  underfunded: { label: 'Underfunded', bg: 'bg-red-50', text: 'text-red-700' },
};
...
function getProgressColor(fundedRatio: number): string {
  if (fundedRatio >= 0.95) return 'bg-emerald-500';
  if (fundedRatio >= 0.5) return 'bg-amber-500';
  return 'bg-red-500';
}

function getProgressTrackColor(fundedRatio: number): string {
  if (fundedRatio >= 0.95) return 'bg-emerald-100';
  if (fundedRatio >= 0.5) return 'bg-amber-100';
  return 'bg-red-100';
}
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <span className="text-xs text-gray-400">Target {goal.targetYear}</span>
          </div>
        </div>
  ...
  if (goals.length === 0) {
    return (
      <div className={className}>
        <p className="text-sm text-gray-400">No goals configured.</p>
      </div>
    );
  }
