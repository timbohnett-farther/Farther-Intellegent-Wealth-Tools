...
function GoalCardItem({ goal }: { goal: GoalCardData }) {
  ...
  return (
    <div className="rounded-2xl border border-border-subtle bg-surface-soft backdrop-blur-xl p-5 shadow-sm">
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-2 min-w-0">
          <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-accent-primary/10">
            <Icon className="h-4 w-4 text-accent-primarySoft" aria-hidden="true" />
          </div>
          <div className="min-w-0">
            <h3 className="text-sm font-semibold text-text truncate">
              {goal.goalName}
            </h3>
            <span className="text-xs text-text-faint">Target {goal.targetYear}</span>
          </div>
        </div>
      ...
      {/* Target amount */}
      <p className="mt-3 text-lg font-bold tabular-nums text-text">
      ...
      <div className="mt-2 flex items-center justify-between">
        <span className="text-xs text-text-faint">Probability</span>
        <span className="text-xs font-medium tabular-nums text-text-muted">
  ...
  if (goals.length === 0) {
    return (
      <div className={className}>
        <p className="text-sm text-text-faint">No goals configured.</p>
      </div>
    );
  }
