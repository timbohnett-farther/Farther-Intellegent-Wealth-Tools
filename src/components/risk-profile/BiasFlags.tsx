...
const SEVERITY_COLORS: Record<string, { bg: string; text: string; dot: string }> = {
  low: { bg: 'bg-accent-primary/10', text: 'text-accent-primarySoft', dot: 'bg-brand-400' },
  ...
  info: { bg: 'bg-accent-primary/10', text: 'text-accent-primarySoft', dot: 'bg-brand-400' },
...
export default function BiasFlags({ biases, inconsistencies }: BiasFlagsProps) {
  if (biases.length === 0 && inconsistencies.length === 0) {
    return (
      <div className="card p-6">
        <h4 className="text-sm font-semibold text-text-muted mb-2">Behavioral Analysis</h4>
  ...
  return (
    <div className="space-y-4">
      ...
      {biases.length > 0 && (
        <div className="card p-6">
          <h4 className="text-sm font-semibold text-text-muted mb-1">Detected Behavioral Biases</h4>
            ...
            {biases.map((bias, i) => {
              ...
              return (
                <div key={i} className={`p-4 rounded-lg ${colors.bg}`}>
                  ...
                  <div className="text-xs text-text-muted bg-text bg-opacity-60 rounded p-2">
      ...
      {inconsistencies.length > 0 && (
        <div className="card p-6">
          <h4 className="text-sm font-semibold text-text-muted mb-1">Profile Inconsistencies</h4>
          <p className="text-xs text-text-faint mb-4">
            ...
            {inconsistencies.map((inc, i) => {
              ...
              return (
                <div key={i} className={`p-4 rounded-lg ${colors.bg}`}>
                  ...
                  <p className="text-xs text-text-muted">{inc.recommendation}</p>
                </div>
              );
            })}
          </div>
