...
const BAND_COLORS: Record<RiskBand, { bg: string; text: string; bar: string }> = {
  1: { bg: 'bg-accent-primary/15', text: 'text-accent-primarySoft', bar: 'from-brand-400 to-brand-700' },
  2: { bg: 'bg-cyan-100', text: 'text-cyan-800', bar: 'from-cyan-400 to-cyan-500' },
  3: { bg: 'bg-accent-primary/15', text: 'text-accent-primarySoft', bar: 'from-brand-400 to-brand-700' },
...
function AxisBar({ label, score }: { label: string; score: number }) {
  return (
    <div className="flex items-center gap-3">
      <span className="text-xs text-text-muted w-28 text-right flex-shrink-0">{label}</span>
      ...
      <span className="text-xs font-semibold text-text w-10">{score}</span>
    </div>
  );
}
...
function GoalFeasibilityRow({ goal }: { goal: GoalFeasibility }) {
  return (
    <div className={`flex items-center justify-between p-3 rounded-lg ${goal.feasible ? 'bg-success-50' : 'bg-warning-50'}`}>
      <div>
        <span className="text-sm font-medium text-text">{goal.goalName}</span>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card p-8 text-center">
        <p className="text-sm text-text-muted mb-3">Recommended Risk Band</p>
      ...
      <div className="card p-6">
        <h4 className="text-sm font-semibold text-text-muted mb-4">Risk Profile Axes</h4>
        ...
        <div className="mt-4 pt-4 border-t border-limestone-100 flex items-center justify-between">
          <span className="text-xs text-text-muted">Required Return to Meet Goals</span>
          <span className="text-sm font-bold text-text">{(axisScores.need * 100).toFixed(1)}%</span>
        </div>
        <div className="flex items-center justify-between mt-1">
          <span className="text-xs text-text-muted">Behavioral Bias Index</span>
          <span className={`text-sm font-bold ${axisScores.biasIndex > 60 ? 'text-warning-500' : axisScores.biasIndex > 30 ? 'text-text-muted' : 'text-success-500'}`}>
      ...
      {profile.goalFeasibility.length > 0 && (
        <div className="card p-6">
          <h4 className="text-sm font-semibold text-text-muted mb-3">Goal Feasibility</h4>
      ...
      <div className="card p-6">
        <div className="flex items-center justify-between mb-1">
          <span className="text-xs text-text-muted">Assessment Confidence</span>
          <span className={`text-sm font-bold ${profile.confidence >= 80 ? 'text-success-500' : profile.confidence >= 60 ? 'text-accent-primarySoft' : 'text-warning-500'}`}>
            {profile.confidence}%
          </span>
        </div>
